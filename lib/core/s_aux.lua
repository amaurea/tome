...
function get_god_level(sch)
	if __schools[sch].gods[player.pgod] then
		...
		-- Hack: Polytheism has access to everything, so use the best version
		local best = nil
		if player.pgod == GOD_POLYTHEISM then
			for god,s in __schools[sch].gods do
				local v = (s_info[s.skill + 1].value * s.mul) / s.div
				if not best or best < v then best = v end
			end
		end
		return best
	end
...
function get_level_school(s, max, min)
	...
	for index, sch in __spell_school[s] do
		...
		if __schools[sch].god then
			if __schools[sch].god ~= player.pgod and player.pgod ~= GOD_POLYTHEISM then
...
function spell_chance(s)
	...
	if get_level_use_stick > -1 then
		chance = lua_spell_device_chance(s_ptr.fail, get_level(s, 50, 0), s_ptr.skill_level)
	else
...
-- Cast a school spell, checking for various failures etc.
-- Returns nil on failure, and TRUE on success.
-- no_cost skips all the checks and costs, and just calls the
-- spell. no_cast skips the spell, and only pays the costs etc.
function cast_school_spell(s, s_ptr, no_cost, no_cast)
	-- Nothing to do
	if no_cost and no_cast then return TRUE end
	...
	local fail = nil
	-- Spell failure?
	if magik(spell_chance(s)) == TRUE then
		fail = TRUE
		local index, sch
	...
	-- Reduce mana first, as the spell might in turn call
	-- other spells.
	adjust_power(s, -get_mana(s))
	-- Take a turn
	if is_magestaff() == TRUE then energy_use = energy_use + 80
	else energy_use = energy_use + 100 end
	...
	-- The "no cost if spell returns nil" part does not work
	-- with no_cast.
	if not no_cast then
		if not __spell_spell[s]() then return end
	end
...
function add_spell_type(t)
	...
	add_hooks
	{
		[HOOK_GF_NAME] = function (gf)
			local t = %t
			if gf == t.index then return TRUE, t.name end
		end,
