...
player.grass_sprout = 0
add_loadsave("player.grass_sprout", 0)
add_hooks
{
	[HOOK_MOVED] = function(oy, ox)
		if player.grass_sprout > 0 then
			grow_grass(get_level(YAVANNA_GROW_GRASS, 4))
			geomancy_random_floor(oy, ox)
			player.grass_sprout = player.grass_sprout - 1
			if player.grass_sprout == 0 then
				msg_print("Grass stops growing around your feet.")
			end
		end
	end
}
YAVANNA_GROW_GRASS = add_spell
{
	...
	["spell"] =     function()
			grow_grass(get_level(YAVANNA_GROW_GRASS, 4))
			if get_level(YAVANNA_GROW_GRASS) >= 10 then
				msg_print("Grass starts growing around your feet.")
				player.grass_sprout = get_level(YAVANNA_GROW_GRASS, 250)
			end
			return TRUE
	end,
	["info"] =      function()
			if get_level(YAVANNA_GROW_GRASS) >= 10 then
				return "rad "..(get_level(YAVANNA_GROW_GRASS, 4)).." dur "..get_level(YAVANNA_GROW_GRASS,250)
			else
	...
	["desc"] =      {
			...
			"At level 10, grass continues growing around your feet for a while."
	}
}
