	...
	["hooks"] =     {
		...
		[HOOK_PLAYER_LEVEL] = function(gained)
			...
			if gained > 0 then
				...
				or ((current_dungeon_idx == god_quest.DUNGEON_GOD) and (dun_level > 0)) or (player.lev <= god_quest.dun_minplev) then
					...
					-- For polytheists, pick a random god
					if player.pgod == GOD_POLYTHEISM then
						local alts = {GOD_ERU,GOD_MANWE,GOD_TULKAS,GOD_MELKOR,GOD_YAVANNA}
						god_quest.current_god = alts[randint(getn(alts))]
					else
						god_quest.current_god = player.pgod
					end

					-- each god has different characteristics, so the quests are differnet depending on your god
					if god_quest.current_god == GOD_ERU then
						god_quest.relic_num = 7
					elseif god_quest.current_god == GOD_MANWE then
						god_quest.relic_num = 8
					elseif god_quest.current_god == GOD_TULKAS then
						god_quest.relic_num = 9
					elseif god_quest.current_god == GOD_MELKOR then
						god_quest.relic_num = 10
					elseif god_quest.current_god == GOD_YAVANNA then
					...
					cmsg_print(TERM_L_BLUE, "The voice of "..deity(god_quest.current_god).name.." booms in your head:")
		...
		[HOOK_GET] = function(o_ptr, item)
			...
			and (o_ptr.pval ~= TRUE)  and (god_quest.relics_found < god_quest.quests_given) then
				...
				cmsg_print(TERM_L_BLUE, deity(god_quest.current_god).name.." speaks to you:")
				...
				if (god_quest.quests_given == god_quest.MAX_NUM_GOD_QUESTS) then
					...
					skill(SKILL_PRAY).raw = skill(SKILL_PRAY).raw + (10 * (skill(SKILL_PRAY).mod))
					...
					skill(SKILL_PRAY).raw = skill(SKILL_PRAY).raw + (5 * (skill(SKILL_PRAY).mod))
				end
...
function get_god_quest_axes()
	...
	if god_quest.current_god ~= GOD_MELKOR then
