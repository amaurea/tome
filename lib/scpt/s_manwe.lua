...
MANWE_SHIELD = add_spell
{
	...
	["spell"] = 	function()
			...
			if get_level(MANWE_SHIELD) >= 10 then
				...
				if get_level(MANWE_SHIELD) >= 40 then
					fire_wave(GF_FORCE, 0, 1, 1+get_level(MANWE_SHIELD, 1)/2, dur, EFF_STORM,get_level(MANWE_SHIELD, 20))
				end
			end
			return obvious
	end,
	["info"] = 	function()
			...
			if get_level(MANWE_SHIELD) >= 40 then
				desc = desc.." rad "..get_level(MANWE_SHIELD, 2).." speed "..get_level(MANWE_SHIELD,20)
			end
			return desc
	end,
	["desc"] =	{
