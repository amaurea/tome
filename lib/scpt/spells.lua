	...
	["hooks"] =
	{
		[HOOK_CALC_MANA] = function(msp)
			if get_skill(SKILL_MANA) >= 35 then
				player.msp = player.msp + (msp * ((get_skill(SKILL_MANA) - 34)) / 100)
			end
		end
	},
}
	...
	["hooks"] =
	{
		[HOOK_CALC_BONUS] = function()
			if get_skill(SKILL_AIR) >= 50 then
				player.magical_breath = player.magical_breath + 1
			end
		end,
		[HOOK_CALC_POWERS] = function()
			if get_skill(SKILL_AIR) >= 50 then
	...
	["hooks"] =
	{
		[HOOK_CALC_BONUS] = function()
			if get_skill(SKILL_WATER) >= 30 then
				player.water_breath = player.water_breath + 1
			end
		end,
		[HOOK_CALC_POWERS] = function()
...
school_book[0] = {
	MANATHRUST, DELCURSES, RESISTS, MANASHIELD, MANAGLOBE,
}
...
school_book[20] = {
	ERU_REMEMBER, ERU_SEE, ERU_LISTEN, ERU_UNDERSTAND, ERU_PROT,
}
