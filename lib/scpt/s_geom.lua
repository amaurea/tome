...
function channel_the_elements(y, x, level, silent)
	local t =
	{
		-- Earth stuff
		[FEAT_GRASS] = function()
			hp_player(player.mhp * (10 + get_skill_scale(SKILL_EARTH, 30)) / 100)
		end,
		[FEAT_FLOWER] = function()
			hp_player(player.mhp * (20 + get_skill_scale(SKILL_EARTH, 60)) / 100)
		end,
		[FEAT_DARK_PIT] = function()
			...
			fire_bolt(type, dir, damroll(%level/3+1, get_skill(SKILL_EARTH)))
		end,
		...
		[FEAT_SHAL_WATER] = function()
			...
			if get_skill(SKILL_WATER) >= 8 then
				fire_beam(type, dir, damroll(%level/10+1, get_skill(SKILL_WATER)))
			else
				fire_bolt(type, dir, damroll(%level/10+1, get_skill(SKILL_WATER)))
			end
		end,
		[FEAT_DEEP_WATER] = function()
			...
			if get_skill(SKILL_WATER) >= 8 then
				fire_beam(type, dir, damroll(%level/5+2, get_skill(SKILL_WATER)))
			else
				fire_bolt(type, dir, damroll(%level/5+2, get_skill(SKILL_WATER)))
			end
		end,
		[FEAT_ICE] = function()
			local ret, dir = get_aim_dir()
			...
			if get_skill(SKILL_WATER) >= 12 then
				fire_ball(GF_ICE, dir, (%level/3+1)*get_skill_scale(SKILL_WATER, 34), 3)
			else
				fire_bolt(GF_ICE, dir, damroll((%level/3)+1, get_skill(SKILL_WATER,34)))
			end
		end,
		...
		[FEAT_SAND] = function()
			...
			set_shield(dur, 0, type, 5 + get_skill_scale(SKILL_FIRE, 20), 5 + %level/2)
			set_blind(dur)
		end,
		[FEAT_SHAL_LAVA] = function()
			...
			if get_skill(SKILL_FIRE) >= 15 then
				fire_bolt(GF_HELL_FIRE, dir, damroll(get_skill_scale(SKILL_FIRE, 30), %level/2+1))
			else
				fire_bolt(GF_FIRE, dir, damroll(get_skill_scale(SKILL_FIRE, 30), %level/2+1))
			end
		end,
		[FEAT_DEEP_LAVA] = function()
			...
			if get_skill(SKILL_FIRE) >= 15 then
				fire_ball(GF_HELL_FIRE, dir, damroll(get_skill_scale(SKILL_FIRE, 30), %level/2+1), 3)
			else
				fire_ball(GF_FIRE, dir, damroll(get_skill_scale(SKILL_FIRE, 30), %level/2+1), 3)
			end
		end,
	}
...
ELEMENTAL_WAVE = add_spell
{
	...
	["spell"] = 	function()
			...
			y = y + player.py
			x = x + player.px
			local level = get_level(ELEMENTAL_WAVE)
			local mul   = level/3+1

			local t =
			{
				-- Earth stuff
				[FEAT_GRASS] 		= { GF_POIS, GF_POIS, 10 + mul*get_skill_scale(SKILL_EARTH, 20) },
				[FEAT_FLOWER] 		= { GF_POIS, GF_POIS, 10 + mul*get_skill_scale(SKILL_EARTH, 30) },
				-- cannot turn chasm into a wave

				-- Water stuff
				[FEAT_SHAL_WATER] 	= { GF_WATER, GF_WATER, 10 + mul*get_skill_scale(SKILL_WATER, 20) },
				[FEAT_DEEP_WATER] 	= { GF_WATER, GF_WATER, 10 + mul*get_skill_scale(SKILL_WATER, 30) },
				[FEAT_ICE] 		= { GF_ICE, GF_ICE, 10 + mul*get_skill_scale(SKILL_WATER, 20) },

				-- Fire stuff
				[FEAT_SAND] 		= { GF_LITE, GF_LITE, 10 + mul*get_skill_scale(SKILL_FIRE, 40) },
				[FEAT_SHAL_LAVA] 	= { GF_FIRE, GF_HOLY_FIRE, 10 + mul*get_skill_scale(SKILL_FIRE, 20) },
				[FEAT_DEEP_LAVA] 	= { GF_FIRE, GF_HOLY_FIRE, 10 + mul*get_skill_scale(SKILL_FIRE, 30) },
			}
...
VAPORIZE = add_spell
{
	...
	["spell"] = 	function()
			local level = get_level(VAPORIZE)
			local mul   = level/2+1
			local t =
			{
				-- Earth stuff
				[FEAT_GRASS] 		= { GF_POIS, GF_POIS, 5 + mul*get_skill_scale(SKILL_EARTH, 10) },
				[FEAT_FLOWER] 		= { GF_POIS, GF_POIS, 5 + mul*get_skill_scale(SKILL_EARTH, 15) },
				[FEAT_DARK_PIT] 	= { GF_DARK, GF_DARK, 5 + mul*get_skill_scale(SKILL_EARTH, 20) },

				-- Water stuff
				[FEAT_SHAL_WATER] 	= { GF_WATER, GF_WATER, 5 + mul*get_skill_scale(SKILL_WATER, 10) },
				[FEAT_DEEP_WATER] 	= { GF_WATER, GF_WATER, 5 + mul*get_skill_scale(SKILL_WATER, 15) },
				[FEAT_ICE] 		= { GF_ICE, GF_ICE, 5 + get_skill_scale(SKILL_WATER, 10) },

				-- Fire stuff
				[FEAT_SAND] 		= { GF_LITE, GF_LITE, 5 + mul*get_skill_scale(SKILL_FIRE, 20) },
				[FEAT_SHAL_LAVA] 	= { GF_FIRE, GF_HOLY_FIRE, 5 + mul*get_skill_scale(SKILL_FIRE, 10) },
				[FEAT_DEEP_LAVA] 	= { GF_FIRE, GF_HOLY_FIRE, 5 + mul*get_skill_scale(SKILL_FIRE, 15) },
			}
