	...
	["spell"] = 	function(flame_item)
			...
			ret, item = get_item("Which object do you want to imbue?",
						...
						if obj.name1 > 0 or obj.name2 > 0 then return FALSE end
							...
							return TRUE
					     	elseif (obj.tval == TV_SHOT) and (obj.sval == SV_AMMO_LIGTH or SV_AMMO_DEATH) then
					     	return TRUE
					     	end
						return FALSE
					     end
...
DEVICE_MANASTORM = add_spell
{
	...
	["spell"] =     function()
		local lvl = get_level(DEVICE_MANASTORM)
		return fire_ball(GF_MANA, 0, 400 + 30*lvl, 4 + lvl/2)
	end,
	["info"] =      function()
		return "dam "..(400 + 30*get_level(DEVICE_MANASTORM)).." rad "..(4 + get_level(DEVICE_MANASTORM)/2)
	end,
	["desc"] =      {
			"Unleashes a powerful storm of mana.",
	}
}

DEVICE_DOOR = add_spell
{
	["name"] =      "Create Doors",
	["school"] =    {SCHOOL_DEVICE},
	["level"] =     10,
	...
	["spell"] =     function()
		local lvl = get_level(DEVICE_DOOR)
		local ret, flg, x, y = get_aim_xy(0, 0, 0)
		if ret == FALSE then return end
		local rad = 0
		local dam = 0
		if lvl >= 10 then flg = bor(flg, PROJECT_BEAM) end
		if lvl >= 20 then flg = bor(flg, PROJECT_WALL) end
		print("A "..lvl)
		return project(0, rad, y, x, dam, GF_MAKE_DOOR, bor(PROJECT_GRID,flg));
	end,
	["info"] =      function()
		return ""
	end,
	["desc"] =      {
			"Creates doors.",
			"At level 10 it creates a row of doors.",
			"At level 20 it can create passages of doors in walls.",
	}
}
