...
function do_melkor_curse(who, level, seen)
	local m_ptr = monster(who)
	local lvl = level or get_level(MELKOR_CURSE)

	if lvl >= 35 then
		-- Not sure how to pass the ego array back from lua to C. Will
		-- hardoce ego to 0 for now
		--local r_ptr = race_info_idx(m_ptr.r_idx, m_ptr.ego)
		local r_ptr = race_info_idx(m_ptr.r_idx, 0)
	...
	if lvl >= 25 then
		m_ptr.speed = m_ptr.speed - lvl*7/50
		m_ptr.mspeed = m_ptr.mspeed - lvl*7/50

		if m_ptr.speed < 70 then m_ptr.speed = 70 end
		if m_ptr.mspeed < 70 then m_ptr.mspeed = 70 end
	end
	if lvl >= 15 then
		m_ptr.ac = m_ptr.ac - lvl
...
MELKOR_MIND_STEAL = add_spell
{
	...
	["spell"] = 	function()
				...
				if target_who == -1 then
					...
					r_ptr = race_info_idx(m_ptr.r_idx, 0)
