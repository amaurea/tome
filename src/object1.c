...
void object_flags(object_type *o_ptr, u32b *f1, u32b *f2, u32b *f3, u32b *f4, u32b *f5, u32b *esp)
{
	...
	/* Random artifact (and egos, it seems) */
	(*f1) |= o_ptr->art_flags1;
	(*f2) |= o_ptr->art_flags2;
	(*f3) |= o_ptr->art_flags3;
	(*f4) |= o_ptr->art_flags4;
...
void object_desc(char *buf, object_type *o_ptr, int pref, int mode)
{
	...
	case TV_TOTEM:
		{
			...
			memset(monster.ego, 0, sizeof(monster.ego));
			monster.ego[0] = o_ptr->pval2;
			monster.ml = TRUE;
	...
	*t = '\0';
	...
	if (known && ((f1 & TR1_PVAL) || (f5 & TR5_PVAL)))
	{
		/* Start the display */
		t = object_desc_chr(t, ' ');
		t = object_desc_chr(t, p1);
 ...
 */
void output_dam(object_type *o_ptr, int nblow, int mult, int mult2, cptr against, cptr against2, bool *first)
{
	int dam;
...
void display_weapon_damage(object_type *o_ptr)
{
	...
	calc_bonuses(TRUE);
	nblow = p_ptr->num_blow_limb[0];

	text_out("\nUsing it you would have ");
	text_out_c(TERM_L_GREEN, format("%d ", nblow));
	text_out(format("blow%s and do an average damage per turn of ", nblow ? "s" : ""));

	if (full && (f1 & TR1_SLAY_ANIMAL)) output_dam(o_ptr, nblow, 2, 0, "animals", NULL, &first);
	if (full && (f1 & TR1_SLAY_EVIL)) output_dam(o_ptr, nblow, 2, 0, "evil creatures", NULL, &first);
	if (full && (f1 & TR1_SLAY_ORC)) output_dam(o_ptr, nblow, 3, 0, "orcs", NULL, &first);
	if (full && (f1 & TR1_SLAY_TROLL)) output_dam(o_ptr, nblow, 3, 0, "trolls", NULL, &first);
	if (full && (f1 & TR1_SLAY_GIANT)) output_dam(o_ptr, nblow, 3, 0, "giants", NULL, &first);
	if (full && (f1 & TR1_KILL_DRAGON)) output_dam(o_ptr, nblow, 5, 0, "dragons", NULL, &first);
	else if (full && (f1 & TR1_SLAY_DRAGON)) output_dam(o_ptr, nblow, 3, 0, "dragons", NULL, &first);
	if (full && (f5 & TR5_KILL_UNDEAD)) output_dam(o_ptr, nblow, 5, 0, "undead", NULL, &first);
	else if (full && (f1 & TR1_SLAY_UNDEAD)) output_dam(o_ptr, nblow, 3, 0, "undead", NULL, &first);
	if (full && (f5 & TR5_KILL_DEMON)) output_dam(o_ptr, nblow, 5, 0, "demons", NULL, &first);
	else if (full && (f1 & TR1_SLAY_DEMON)) output_dam(o_ptr, nblow, 3, 0, "demons", NULL, &first);

	if (full && (f1 & TR1_BRAND_FIRE)) output_dam(o_ptr, nblow, 3, 6, "non fire resistant creatures", "fire susceptible creatures", &first);
	if (full && (f1 & TR1_BRAND_COLD)) output_dam(o_ptr, nblow, 3, 6, "non cold resistant creatures", "cold susceptible creatures", &first);
	if (full && (f1 & TR1_BRAND_ELEC)) output_dam(o_ptr, nblow, 3, 6, "non lightning resistant creatures", "lightning susceptible creatures", &first);
	if (full && (f1 & TR1_BRAND_ACID)) output_dam(o_ptr, nblow, 3, 6, "non acid resistant creatures", "acid susceptible creatures", &first);
	if (full && (f1 & TR1_BRAND_POIS)) output_dam(o_ptr, nblow, 3, 6, "non poison resistant creatures", "poison susceptible creatures", &first);

	output_dam(o_ptr, nblow, 1, 0, (first) ? "all monsters" : "other monsters", NULL, &first);
...
void display_ammo_damage(object_type *o_ptr)
{
	...
	if (o_ptr->pval2)
	{
		char * name = get_gf_name(o_ptr->pval2);
		text_out("The explosion will be ");
		text_out_c(TERM_L_GREEN, name ? name : "something weird");
		text_out(".");
	}
}
...
bool object_out_desc(object_type *o_ptr, FILE *fff, bool trim_down, bool wait_for_it)
{
	...
	if (object_known_p(o_ptr))
	{
		...
		if (f4 & TR4_ULTIMATE)
		{
			if ((wield_slot(o_ptr) == INVEN_WIELD) ||
			                (wield_slot(o_ptr) == INVEN_BOW) || (wield_slot(o_ptr) == INVEN_AMMO))
		...
		if(f5 & TR5_RETURNING) text_out("It returns to you after being fired.  ");
		if(f5 & TR5_INSULATE) text_out("It resists being drained.");
		...
		{
			if (f2 & (TRAP2_AUTOMATIC_5)) text_out("It can rearm itself.  ");
			if (f2 & (TRAP2_AUTOMATIC_99))text_out("It rearms itself.  ");
			if (f2 & (TRAP2_KILL_GHOST))  text_out("It is effective against Ghosts.  ");
			if (f2 & (TRAP2_TELEPORT_TO)) text_out("It can teleport monsters to you.  ");
			if (f2 & (TRAP2_ONLY_DRAGON)) text_out("It can only be set off by dragons.  ");
			if (f2 & (TRAP2_ONLY_DEMON))  text_out("It can only be set off by demons.  ");
			if (f2 & (TRAP2_ONLY_UNDEAD)) text_out("It can only be set off by undead.  ");
			if (f2 & (TRAP2_ONLY_ANIMAL)) text_out("It can only be set off by animals.  ");
			if (f2 & (TRAP2_ONLY_EVIL))   text_out("It can only be set off by evil creatures.  ");
		}
		else if (o_ptr->tval == TV_RUNE_ELEM || o_ptr->tval == TV_RUNE_SHAPE || o_ptr->tval == TV_RUNE_POWER)
		{
			if (f2 & RUNE2_HIT_SELF)     text_out("It targets yourself.  ");
			if (f2 & RUNE2_HIT_GROUND)   text_out("It affetcs the ground.  ");
			if (f2 & RUNE2_CAN_AIM)      text_out("It can be aimed.  ");
			if (f2 & RUNE2_BEAM)         text_out("It creates a beam effect.  ");
			if (f2 & RUNE2_BALL)         text_out("It creates a ball effect.  ");
			if (f2 & RUNE2_SHOWER)       text_out("It creates a storm of explosions.  ");
			if (f2 & RUNE2_VIEW)         text_out("It targets everything in sight.  ");
			if (f2 & RUNE2_STAY)         text_out("It stays in effect for several rounds.  ");
			if (f2 & RUNE2_WAVE)         text_out("It creates a wave effect.  ");
			if (f2 & RUNE2_STORM)        text_out("It surrounds you as a storm.  ");
		}
		else
		{
			vn = 0;
	                ...
	                !trim_down || (ego_item_p(o_ptr)) || (artifact_p(o_ptr)))
	{
		...
		{
			char m_name[80];
			u16b tmp[2] = { o_ptr->found_aux2, 0 };

			monster_race_desc(m_name, o_ptr->found_aux1, tmp);
