...
/*
 * The difficulty should be set on the trap when it is placed,
 * instead of being calculated when one tries to disarm it.
 * However, this entails changes to the cave pointer and thereby
 * changes to the save file.
*/
static bool check_disarm(int difficulty, int skill)
{
	int i, mul;
	if(skill <= 0) skill = 1; /* (very) small chance for everybody */
	mul = (skill+7-1)/7;
	for(i = 0; i < mul; i++)
		if(rand_int(dun_level+difficulty+2*skill) > dun_level+difficulty)
			return TRUE;
	return FALSE;
}
...
static bool do_cmd_open_aux(int y, int x, int dir)
{
	...
	else if (c_ptr->feat >= FEAT_DOOR_HEAD + 0x01)
	{
		...
		/* Success */
		if(check_disarm(i,4*(c_ptr->feat - FEAT_DOOR_HEAD)))
		{
			/* Message */
			msg_print("You have picked the lock.");
...
static bool do_cmd_disarm_chest(int y, int x, s16b o_idx)
{
	...
	/* Penalize some conditions */
	if (p_ptr->blind || no_lite()) i /= 5;
	if (p_ptr->confused || p_ptr->image) i /= 5;
	...
	else if (check_disarm(t_ptr->difficulty, i))
	{
		msg_print("You have disarmed the chest.");
		gain_exp((t_ptr->difficulty+dun_level) * (t_ptr->difficulty+dun_level)/10 * 3);
...
#endif /* ALLOW_EASY_DISARM -- TNB */
{
	...
	/* Success */
	if (check_disarm(power, i))
	{
		/* Message */
		msg_format("You have disarmed the %s.", name);

		/* Reward */
		gain_exp((power+dun_level) * (power+dun_level)/10);
...
void do_cmd_fire(void)
{
	...
	if(!(f5 & TR5_RETURNING)) dec_item_helper(item);
	artifact = q_ptr->art_name || artifact_p(q_ptr);
	...
	while (TRUE)
	{
		...
		/* Break ? */
		if ((q_ptr->pval2 != 0 || rand_int(100) < j ) && !artifact)
		{
			breakage = 100;
			break;
		}
	...
	if(f5 & TR5_RETURNING && !artifact && (rand_int(100) < breakage))
		{
			dec_item_helper(item);
			breakage = 100; /* HACK! */
		}
	if(!(f5 & TR5_RETURNING)) drop_near(q_ptr, breakage, y, x);
}
...
void do_cmd_throw(void)
{
	...
	/* Drop (or break) near that location */
	if(f5 & TR5_RETURNING && !(q_ptr->art_name || artifact_p(q_ptr)) && (rand_int(100) < j))
		{
			dec_item_helper(item);
			j = 100; /* HACK! */
		}
	if(!(f5 & TR5_RETURNING)) drop_near(q_ptr, j, y, x);
}
...
byte show_monster_inven(int m_idx, int *monst_list)
{
	...
	int col, len, lim, ci, tail_len, show_steal = 1;
	...
	tail_len = 0;
	if (show_weights) tail_len += 9;
	if (show_steal)   tail_len += 9;
	lim -= tail_len;
	...
	for (j = 0; j < k; j++)
	{
		...
		ci = col;
		put_str(tmp_val, j + 1, ci);
		ci += 3;

		/* Display the entry itself */
		c_put_str(out_color[j], out_desc[j], j + 1, ci);
		ci += len-tail_len;
		...
		if (show_steal)
		{
			int difficulty = 40 - p_ptr->stat_ind[A_DEX] +
				o_ptr->weight*o_ptr->number / (get_skill_scale(SKILL_STEALING, 19) + 1) -
				(get_skill_scale(SKILL_STEALING, 29) + 1) -
				(m_ptr->csleep ? 10 + m_ptr->level/2 : 0) +
				m_ptr->level;
			int chance = difficulty > 0 ? 100*(1+get_skill_scale(SKILL_STEALING, 25))/difficulty : 100;
			if(chance > 100) chance = 100;
			strnfmt(tmp_val, 80, " %3d%%", chance);
			c_put_str(TERM_BLUE, tmp_val, j+1, ci);
			ci += 5;
		}
	}
...
void do_cmd_steal()
{
	int x, y, dir = 0, item = -1, k = -1, chance;
	...
	/* Made stealing form sleeping monsters easier at high levels.
	 * For stealing a weightless item from an awake monster, with
	 * max dex and skill, the threshold of 100% success is lvl 63,
	 * and at lvl 100, this has fallen to 41%. If the monster is
	 * asleep, you are now basically guaranteed to be able to steal
	 * from it if you have maxed stealing.
	 */
	chance = 40 - p_ptr->stat_ind[A_DEX];
	chance += o_list[item].weight*o_list[item].number / (get_skill_scale(SKILL_STEALING, 19) + 1);
	chance -= get_skill_scale(SKILL_STEALING, 29) + 1; /* was plus */
	chance -= m_ptr->csleep ? 10 + m_ptr->level/2 : 0;
	chance += m_ptr->level;
	...
	if (PRACE_FLAGS(PR1_EASE_STEAL))
	{
		...
		/* Max XP gained from stealing. Ama scaled this up a bit so that
		 * stealing still gives some XP later in the game. A great bile
		 * wyrm is lvl 67 and gives 23000 XP. With the old formula,
		 * one would get 670 XP for stealing from it. With the new formula,
		 * one gets 9380 XP. At that point, the chance of succeeding is
		 * */
		max_point = o_list[item].weight*o_list[item].number/2 + m_ptr->level*(1 + m_ptr->level/5)*10;
