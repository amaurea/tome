...
static void prt_lasting_effect(void)
{
	call_lua("print_effect_summary", "(d,d)", "", ROW_EFFECT, COL_EFFECT);
}
...
static void prt_sp(void)
{
	char tmp[32];
	byte color;
	int spectre = PRACE_FLAG(PR1_SEMI_WRAITH);

	if (player_char_health && spectre) lite_spot(p_ptr->py, p_ptr->px);
...
{
	int msp, levels, cur_wgt, max_wgt, hpsphack;
	u32b f1, f2, f3, f4, f5, esp;

	/* Spectres are different. Without checking for undeath,
	 * specters have a very hard time as necromancers. With
	 * this check, they have it much easier than others.
	 * Decreasing the amount of DP could help a bit, I guess,
	 * but with high enough speed, even 1 DP would be possible
	 * to live with, so I'm not sure what to do. */
	hpsphack = PRACE_FLAG(PR1_SEMI_WRAITH) && !(p_ptr->necro_extra & CLASS_UNDEAD);
	...
	if (p_ptr->body_monster && (!p_ptr->disembodied))
	{
		...
		msp = msp*r_ptr->freq_spell/25; /* Normal player counted as 1 in 4 */
		if (msp < 1) msp = 1;
	}
	if (p_ptr->mimic_form) hpsphack = FALSE;
	...
	/* Sp mods? */
	p_ptr->msp = msp;
	process_hooks(HOOK_CALC_MANA, "(d)", msp);
	msp = p_ptr->msp;

	/* Mana can never be negative */
	if (msp < 0) msp = 0;

	/* hack hack! This depends on hp being processed first. */
	if(hpsphack)
	{
		msp += p_ptr->mhp;
		if(p_ptr->mhp > 1) p_ptr->redraw |= (PR_HP);
		p_ptr->mhp = 1;
		if(p_ptr->chp > p_ptr->mhp) p_ptr->chp = 1;
	}
...
void calc_body_bonus()
{
	...
	if (r_ptr->flags1 & RF1_NEVER_MOVE) p_ptr->immovable++;
	if (r_ptr->flags2 & RF2_STUPID) p_ptr->stat_add[A_INT] -= 1;
	if (r_ptr->flags2 & RF2_SMART) p_ptr->stat_add[A_INT] += 1;
	if (r_ptr->flags2 & RF2_REFLECTING) p_ptr->reflect++;
	if (r_ptr->flags2 & RF2_INVISIBLE) p_ptr->invis += 20;
	if (r_ptr->flags2 & RF2_REGENERATE) p_ptr->regenerate++;
	if (r_ptr->flags2 & RF2_AURA_FIRE) p_ptr->sh_fire++;
	if (r_ptr->flags2 & RF2_AURA_ELEC) p_ptr->sh_elec++;
	if (r_ptr->flags2 & RF2_PASS_WALL) p_ptr->wraith_form++;
	if (r_ptr->flags3 & RF3_SUSCEP_FIRE) p_ptr->sensible_fire++;
	if (r_ptr->flags3 & RF3_IM_ACID) p_ptr->resist_acid++;
	if (r_ptr->flags3 & RF3_IM_ELEC) p_ptr->resist_elec++;
	if (r_ptr->flags3 & RF3_IM_FIRE) p_ptr->resist_fire++;
	if (r_ptr->flags3 & RF3_IM_POIS) p_ptr->resist_pois++;
	if (r_ptr->flags3 & RF3_IM_COLD) p_ptr->resist_cold++;
	if (r_ptr->flags3 & RF3_RES_NETH) p_ptr->resist_neth++;
	if (r_ptr->flags3 & RF3_RES_NEXU) p_ptr->resist_nexus++;
	if (r_ptr->flags3 & RF3_RES_DISE) p_ptr->resist_disen++;
	if (r_ptr->flags3 & RF3_NO_FEAR) p_ptr->resist_fear++;
	if (r_ptr->flags3 & RF3_NO_SLEEP) p_ptr->free_act++;
	if (r_ptr->flags3 & RF3_NO_CONF) p_ptr->resist_conf++;
	if (r_ptr->flags7 & RF7_CAN_FLY) p_ptr->ffall++;
	if (r_ptr->flags7 & RF7_AQUATIC) p_ptr->water_breath++;
}
...
void calc_gods()
{
	...
	GOD(GOD_MELKOR)
	{
		...
		PRAY_GOD(GOD_MELKOR)
		{
			...
			if (p_ptr->grace > 15000) p_ptr->immune_fire++;
		}
		p_ptr->resist_fire++;
	}
	...
	PRAY_GOD(GOD_MANWE)
	{
		...
		/* Provides fly & FA */
		if (p_ptr->grace >= 7000) p_ptr->free_act++;
		if (p_ptr->grace >= 15000) p_ptr->fly++;
	}

	/* Manwe bonus not requiring the praying status */
	GOD(GOD_MANWE)
	{
		if (p_ptr->grace >= 2000) p_ptr->ffall++;
	}
...
void apply_flags(u32b f1, u32b f2, u32b f3, u32b f4, u32b f5, u32b esp, s16b pval, s16b tval, s16b to_h, s16b to_d, s16b to_a)
{
	...
	/* Hack -- Sensible fire */
	if (f2 & (TR2_SENS_FIRE)) p_ptr->sensible_fire++;

	/* Hack -- cause earthquakes */
	if (f1 & (TR1_IMPACT)) p_ptr->impact++;
	...
	/* Various flags */
	if (f3 & (TR3_AGGRAVATE)) p_ptr->aggravate++;
	if (f3 & (TR3_TELEPORT)) p_ptr->teleport++;
	if (f5 & (TR5_DRAIN_MANA)) p_ptr->drain_mana++;
	if (f5 & (TR5_DRAIN_HP)) p_ptr->drain_life++;
	if (f3 & (TR3_DRAIN_EXP)) p_ptr->exp_drain++;
	if (f3 & (TR3_BLESSED)) p_ptr->bless_blade++;
	if (f3 & (TR3_XTRA_MIGHT)) p_ptr->xtra_might += pval;
	if (f3 & (TR3_SLOW_DIGEST)) p_ptr->slow_digest++;
	if (f3 & (TR3_REGEN)) p_ptr->regenerate++;
	if (esp) p_ptr->telepathy |= esp;
	if ((tval != TV_LITE) && (f3 & (TR3_LITE1))) p_ptr->lite++;
	if ((tval != TV_LITE) && (f4 & (TR4_LITE2))) p_ptr->lite++;
	if ((tval != TV_LITE) && (f4 & (TR4_LITE3))) p_ptr->lite++;
	if (f3 & (TR3_SEE_INVIS)) p_ptr->see_inv++;
	if (f2 & (TR2_FREE_ACT)) p_ptr->free_act++;
	if (f2 & (TR2_HOLD_LIFE)) p_ptr->hold_life++;
	if (f3 & (TR3_WRAITH)) p_ptr->wraith_form++;
	if (f3 & (TR3_FEATHER)) p_ptr->ffall++;
	if (f4 & (TR4_FLY)) p_ptr->fly++;
	if (f4 & (TR4_CLIMB)) p_ptr->climb++;

	/* Immunity flags */
	if (f2 & (TR2_IM_FIRE)) p_ptr->immune_fire++;
	if (f2 & (TR2_IM_ACID)) p_ptr->immune_acid++;
	if (f2 & (TR2_IM_COLD)) p_ptr->immune_cold++;
	if (f2 & (TR2_IM_ELEC)) p_ptr->immune_elec++;

	/* Resistance flags */
	if (f2 & (TR2_RES_ACID)) p_ptr->resist_acid++;
	if (f2 & (TR2_RES_ELEC)) p_ptr->resist_elec++;
	if (f2 & (TR2_RES_FIRE)) p_ptr->resist_fire++;
	if (f2 & (TR2_RES_COLD)) p_ptr->resist_cold++;
	if (f2 & (TR2_RES_POIS)) p_ptr->resist_pois++;
	if (f2 & (TR2_RES_FEAR)) p_ptr->resist_fear++;
	if (f2 & (TR2_RES_CONF)) p_ptr->resist_conf++;
	if (f2 & (TR2_RES_SOUND)) p_ptr->resist_sound++;
	if (f2 & (TR2_RES_LITE)) p_ptr->resist_lite++;
	if (f2 & (TR2_RES_DARK)) p_ptr->resist_dark++;
	if (f2 & (TR2_RES_CHAOS)) p_ptr->resist_chaos++;
	if (f2 & (TR2_RES_DISEN)) p_ptr->resist_disen++;
	if (f2 & (TR2_RES_SHARDS)) p_ptr->resist_shard++;
	if (f2 & (TR2_RES_NEXUS)) p_ptr->resist_nexus++;
	if (f2 & (TR2_RES_BLIND)) p_ptr->resist_blind++;
	if (f2 & (TR2_RES_NETHER)) p_ptr->resist_neth++;
	if (f4 & (TR4_IM_NETHER)) p_ptr->immune_neth++;

	if (f2 & (TR2_REFLECT)) p_ptr->reflect++;
	if (f3 & (TR3_SH_FIRE)) p_ptr->sh_fire++;
	if (f3 & (TR3_SH_ELEC)) p_ptr->sh_elec++;
	if (f3 & (TR3_NO_MAGIC)) p_ptr->anti_magic++;
	if (f3 & (TR3_NO_TELE)) p_ptr->anti_tele++;

	/* Sustain flags */
	if (f2 & (TR2_SUST_STR)) p_ptr->sustain_str++;
	if (f2 & (TR2_SUST_INT)) p_ptr->sustain_int++;
	if (f2 & (TR2_SUST_WIS)) p_ptr->sustain_wis++;
	if (f2 & (TR2_SUST_DEX)) p_ptr->sustain_dex++;
	if (f2 & (TR2_SUST_CON)) p_ptr->sustain_con++;
	if (f2 & (TR2_SUST_CHR)) p_ptr->sustain_chr++;

	if (f4 & (TR4_PRECOGNITION)) p_ptr->precognition++;
	 ...
	 * an undead.  -LM-
	 */
	if (f4 & (TR4_BLACK_BREATH)) p_ptr->black_breath++;

	if (f5 & (TR5_IMMOVABLE)) p_ptr->immovable++;

	/* Breaths */
	if (f5 & (TR5_WATER_BREATH)) p_ptr->water_breath++;
	if (f5 & (TR5_MAGIC_BREATH))
	{
		p_ptr->magical_breath++;
		p_ptr->water_breath++;
	}
}

u32b flag2int(u32b flag)
{
	u32b i;
	for(i = 0; !(flag&1); i++, flag>>=1);
	return i;
}

/* The arguments are *arrays* rather than flag sets here. The function fills
 * them with the numeric bonus each of them has */
void get_effective_flags(int * f1, int * f2, int * f3, int * f4, int * f5, int * esp)
{
	int i, lite;
	for(i = 0; i < 32; i++)
		f1[i] = f2[i] = f3[i] = f4[i] = f5[i] = esp[i] = 0;
	f1[flag2int(TR1_STR)] = p_ptr->stat_add[A_STR];
	f1[flag2int(TR1_INT)] = p_ptr->stat_add[A_INT];
	f1[flag2int(TR1_WIS)] = p_ptr->stat_add[A_WIS];
	f1[flag2int(TR1_DEX)] = p_ptr->stat_add[A_DEX];
	f1[flag2int(TR1_CON)] = p_ptr->stat_add[A_CON];
	f1[flag2int(TR1_CHR)] = p_ptr->stat_add[A_CHR];
	f5[flag2int(TR5_LUCK)] = p_ptr->luck_cur;

	/* Affect spell power */
	f1[flag2int(TR1_SPELL)] = p_ptr->to_s;

	/* Affect mana capacity */
	f1[flag2int(TR1_MANA)] = p_ptr->to_m;

	/* Affect life capacity */
	f2[flag2int(TR2_LIFE)] = p_ptr->to_l;

	/* Affect stealth */
	f1[flag2int(TR1_STEALTH)] = p_ptr->skill_stl;

	/* Affect searching ability (factor of five) */
	f1[flag2int(TR1_SEARCH)] = p_ptr->skill_srh/5;

	/* Affect searching frequency (factor of five) */

	/* Affect infravision */
	f1[flag2int(TR1_INFRA)] = p_ptr->see_infra;

	/* Affect digging (factor of 20) */
	f1[flag2int(TR1_TUNNEL)] = p_ptr->skill_dig/20;

	/* Affect speed */
	f1[flag2int(TR1_SPEED)] = p_ptr->pspeed-110;

	/* Affect blows */
	f1[flag2int(TR1_BLOWS)] = extra_blows;
	f5[flag2int(TR5_CRIT)]  = p_ptr->xtra_crit;

	/* Hack -- Sensitive to fire */
	f2[flag2int(TR2_SENS_FIRE)] = p_ptr->sensible_fire;

	/* Hack -- cause earthquakes */
	f1[flag2int(TR1_IMPACT)] = p_ptr->impact;

	/* Affect invisibility */
	f2[flag2int(TR2_INVIS)] = p_ptr->invis/10;

	/* Boost shots */
	f3[flag2int(TR3_XTRA_SHOTS)] = extra_shots;

	/* Various flags */
	f3[flag2int(TR3_AGGRAVATE)]     = p_ptr->aggravate;
	f3[flag2int(TR3_TELEPORT)]      = p_ptr->teleport;
	f5[flag2int(TR5_DRAIN_MANA)]    = p_ptr->drain_mana;
	f5[flag2int(TR5_DRAIN_HP)]      = p_ptr->drain_life;
	f3[flag2int(TR3_DRAIN_EXP)]     = p_ptr->exp_drain;
	f3[flag2int(TR3_BLESSED)]       = p_ptr->bless_blade;
	f3[flag2int(TR3_XTRA_MIGHT)]    = p_ptr->xtra_might;
	f3[flag2int(TR3_SLOW_DIGEST)]   = p_ptr->slow_digest;
	f3[flag2int(TR3_REGEN)]         = p_ptr->regenerate;
	for(i=0; i < 32; i++)
		esp[i] = !!(p_ptr->telepathy & (1<<i));
	lite = p_ptr->cur_lite;
	if(lite >= 3) { f4[flag2int(TR4_LITE3)] = 1; lite -= 3; }
	if(lite >= 2) { f4[flag2int(TR4_LITE2)] = 1; lite -= 2; }
	if(lite >= 1) { f3[flag2int(TR3_LITE1)] = 1; lite -= 1; }
	f3[flag2int(TR3_SEE_INVIS)]     = p_ptr->see_inv;
	f2[flag2int(TR2_FREE_ACT)]      = p_ptr->free_act;
	f2[flag2int(TR2_HOLD_LIFE)]     = p_ptr->hold_life;
	f3[flag2int(TR3_WRAITH)]        = p_ptr->wraith_form;
	f3[flag2int(TR3_FEATHER)]       = p_ptr->ffall;
	f4[flag2int(TR4_FLY)]           = p_ptr->fly;
	f4[flag2int(TR4_CLIMB)]         = p_ptr->climb;

	/* Immunity flags */
	f2[flag2int(TR2_IM_FIRE)] = p_ptr->immune_fire;
	f2[flag2int(TR2_IM_ACID)] = p_ptr->immune_acid;
	f2[flag2int(TR2_IM_COLD)] = p_ptr->immune_cold;
	f2[flag2int(TR2_IM_ELEC)] = p_ptr->immune_elec;

	/* Resistance flags */
	f2[flag2int(TR2_RES_ACID)]   = p_ptr->resist_acid;
	f2[flag2int(TR2_RES_ELEC)]   = p_ptr->resist_elec;
	f2[flag2int(TR2_RES_FIRE)]   = p_ptr->resist_fire;
	f2[flag2int(TR2_RES_COLD)]   = p_ptr->resist_cold;
	f2[flag2int(TR2_RES_POIS)]   = p_ptr->resist_pois;
	f2[flag2int(TR2_RES_FEAR)]   = p_ptr->resist_fear;
	f2[flag2int(TR2_RES_CONF)]   = p_ptr->resist_conf;
	f2[flag2int(TR2_RES_SOUND)]  = p_ptr->resist_sound;
	f2[flag2int(TR2_RES_LITE)]   = p_ptr->resist_lite;
	f2[flag2int(TR2_RES_DARK)]   = p_ptr->resist_dark;
	f2[flag2int(TR2_RES_CHAOS)]  = p_ptr->resist_chaos;
	f2[flag2int(TR2_RES_DISEN)]  = p_ptr->resist_disen;
	f2[flag2int(TR2_RES_SHARDS)] = p_ptr->resist_shard;
	f2[flag2int(TR2_RES_NEXUS)]  = p_ptr->resist_nexus;
	f2[flag2int(TR2_RES_BLIND)]  = p_ptr->resist_blind;
	f2[flag2int(TR2_RES_NETHER)] = p_ptr->resist_neth;
	f4[flag2int(TR4_IM_NETHER)]  = p_ptr->immune_neth;

	f2[flag2int(TR2_REFLECT)]    = p_ptr->reflect;
	f3[flag2int(TR3_SH_FIRE)]    = p_ptr->sh_fire;
	f3[flag2int(TR3_SH_ELEC)]    = p_ptr->sh_elec;
	f3[flag2int(TR3_NO_MAGIC)]   = p_ptr->anti_magic;
	f3[flag2int(TR3_NO_TELE)]    = p_ptr->anti_tele;

	/* Sustain flags */
	f2[flag2int(TR2_SUST_STR)]   = p_ptr->sustain_str;
	f2[flag2int(TR2_SUST_INT)]   = p_ptr->sustain_int;
	f2[flag2int(TR2_SUST_WIS)]   = p_ptr->sustain_wis;
	f2[flag2int(TR2_SUST_DEX)]   = p_ptr->sustain_dex;
	f2[flag2int(TR2_SUST_CON)]   = p_ptr->sustain_con;
	f2[flag2int(TR2_SUST_CHR)]   = p_ptr->sustain_chr;

	f4[flag2int(TR4_PRECOGNITION)] = p_ptr->precognition;

	f4[flag2int(TR4_ANTIMAGIC_50)] = !!get_skill_scale(SKILL_ANTIMAGIC, 40);
	f4[flag2int(TR4_AUTO_ID)]    = p_ptr->auto_id;
	 ...
	 * an undead.  -LM-
	 */
	f4[flag2int(TR4_BLACK_BREATH)] = p_ptr->black_breath;
	f5[flag2int(TR5_IMMOVABLE)]    = p_ptr->immovable;

	/* Breaths */
	f5[flag2int(TR5_WATER_BREATH)] = p_ptr->water_breath;
	f5[flag2int(TR5_MAGIC_BREATH)] = p_ptr->magical_breath;

	/* Mark the purely boolean flags as such */
	for(i=0;i<32;i++)
	{
		if(f1[i]  && !(TR1_PVAL&(1<<i))) f1[i]  <<= 16;
		if(f2[i]  && !(TR2_PVAL&(1<<i))) f2[i]  <<= 16;
		if(f3[i]  && !(TR3_PVAL&(1<<i))) f3[i]  <<= 16;
		if(f4[i]  && !(TR4_PVAL&(1<<i))) f4[i]  <<= 16;
		if(f5[i]  && !(TR5_PVAL&(1<<i))) f5[i]  <<= 16;
		if(esp[i] && !(ESP_PVAL&(1<<i))) esp[i] <<= 16;
	}
}
...
void calc_bonuses(bool silent)
{
	...
	if (get_skill(SKILL_ANTIMAGIC))
	{
		...
		{
			p_ptr->anti_tele++;
			p_ptr->resist_continuum++;
		}
	}

	if (get_skill(SKILL_DAEMON) > 20) p_ptr->resist_conf++;
	if (get_skill(SKILL_DAEMON) > 30) p_ptr->resist_fear++;
	...
	/* Assume no hits */
	for(i = 0; p_ptr->body_parts[i] == INVEN_WIELD; i++)
		p_ptr->num_blow_limb[i] = 0;
	...
	if (p_ptr->melee_style == SKILL_MASTERY)
	{
		for(i = 0; p_ptr->body_parts[i] == INVEN_WIELD; i++)
		{
			...
			p_ptr->num_blow_limb[i] = 1;
			o_ptr = &p_ptr->inventory[INVEN_WIELD+i];

			if (!o_ptr->k_idx || p_ptr->heavy_wield) continue;
			...
			/* Use the blows table */
			p_ptr->num_blow_limb[i] = blows_table[str_index][dex_index];

			/* Maximal value */
			if (p_ptr->num_blow_limb[i] > num) p_ptr->num_blow_limb[i] = num;

			/* Add in the "bonus blows" */
			p_ptr->num_blow_limb[i] += extra_blows;

			/* Special class bonus blows */
			p_ptr->num_blow_limb[i] += p_ptr->lev * cp_ptr->extra_blows / 50;

			/* Weapon specialization bonus blows */
			if (get_weaponmastery_skill() != -1)
				p_ptr->num_blow_limb[i] += get_skill_scale(get_weaponmastery_skill(), 2);

			/* Bonus blows for plain weaponmastery skill */
			p_ptr->num_blow_limb[i] += get_skill_scale(SKILL_MASTERY, 3);

			/* Require at least one blow */
			if (p_ptr->num_blow_limb[i] < 1) p_ptr->num_blow_limb[i] = 1;

			p_ptr->num_blow += p_ptr->num_blow_limb[i];
		}
	}
	/* Different calculation for monk with empty hands */
	else if ((p_ptr->melee_style == SKILL_HAND) && monk_empty_hands())
	{
		...
		p_ptr->num_blow_limb[0] = p_ptr->num_blow;
	...
	/* Different calculation for beornings */
	...
	p_ptr->skill_dis += (get_skill_scale(SKILL_SNEAK, 50));
	...
	p_ptr->skill_srh += (get_skill_scale(SKILL_SNEAK, 50));

	/* Affect Skill -- search frequency (Sneakiness skill) */
	p_ptr->skill_fos += (get_skill_scale(SKILL_SNEAK, 100));

	/* Affect Skill -- combat (Combat skill + mastery) */
	p_ptr->skill_thn += 50 * (7*get_skill(p_ptr->melee_style) + 3*get_skill(SKILL_COMBAT)) / 100;

	/* Affect Skill -- combat (shooting) (Level, by Class) */
	p_ptr->skill_thb += 50 * (7 * get_skill(SKILL_ARCHERY) + 3 * get_skill(SKILL_COMBAT)) / 100;
...
void update_stuff(void)
{
	...
	/* hpsphacksaker very ugly */
	if(PRACE_FLAG(PR1_SEMI_WRAITH))
	{
		if(p_ptr->update & (PU_MANA)) p_ptr->update |= (PU_HP);
		if(p_ptr->update & (PU_HP)) p_ptr->update |= (PU_MANA);
	}
...
void redraw_stuff(void)
{
	...
	if(p_ptr->redraw & PR_SPEED) prt_frame_extra();
