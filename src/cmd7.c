...
{
	int plev = get_skill(SKILL_MINDCRAFT), dam, rad;
	...
	case 7:
		break;
	case 8:
		dam = plev*(plev-5)*(plev+50)/500+1;
		rad = 2+(plev-15)/2;
		if (plev < 25)
			strnfmt(p, 80, " dam %d rad %d", dam, rad);
		else
			strnfmt(p, 80, " dam %d", dam);
		break;
	case 9:
	...
	case 10:
		...
		break;
	case 11:
		strnfmt(p, 80, " dam %d rad %d", plev * (plev > 39 ? 7 : 3), 3 + plev / 10);
		break;
	}
}
...
void do_cmd_mindcraft(void)
{
		...
		case 8:
			{
				...
				dam = plev*(plev-5)*(plev+50)/500+1;
				rad = 2+(plev-15)/2;
				if (plev < 20)     project(0, rad, p_ptr->py, p_ptr->px, dam, GF_PSI, PROJECT_KILL);
				else if(plev < 25) project_meteor(rad, GF_PSI, dam, PROJECT_KILL);
				else               mindblast_monsters(dam);
				break;
			}
		...
		case 11:
			{
				...
				project_meteor(3 + plev / 10, GF_TELEKINESIS,
				        plev * (plev > 39 ? 7 : 3),
	...
	/* Sufficient mana */
	if (spell.mana_cost <= p_ptr->csp) modify_sp(-spell.mana_cost);
...
void do_cmd_mimic_lore()
{
	...
	else
	{
		...
		else
		{
			...
			set_mimic(-1, o_ptr->pval2, get_skill(SKILL_MIMICRY));
		}
	}
		...
		case 2:
			{
				...
				if (att & CLASS_LEGS)
				{
					msg_print("You mimic away your extra pair of legs.");
					att ^= (CLASS_LEGS);
				}
				else
				{
		...
		case 3:
			{
				...
				if (att & CLASS_WALL)
				{
					msg_print("You lose your affinity for walls.");
					att ^= CLASS_WALL;
				}
				else
				{
		...
		case 4:    /* Arms Mimicry */
			{
				...
				if (att & CLASS_ARMS)
				{
					msg_print("You mimic away your extra pair of arms.");
					att ^= CLASS_ARMS;
				}
				else
				{
		...
		case 4:
			{
				int i, dam, flg;
				if (!get_aim_dir(&dir)) return;
				dam = (1 + to_s2 + (plev / 15)) * 100;
				flg = PROJECT_STOP | PROJECT_KILL;
				if(plev < 40) project_hook(GF_OLD_DRAIN, dir, dam, flg);
				else project_hook(GF_UNHOLY_DRAIN, dir, dam, flg);
		...
		case 3:
			{
				...
				set_hp((percent1 * p_ptr->mhp) / 100, "sharing life");
