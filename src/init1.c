...
static errr grab_one_race_kind_flag(u32b *f1, u32b *f2, u32b *f3, u32b *f4, u32b *f5, u32b *esp, cptr what)
{
	...
	for (i = 0; i < 32; i++)
	{
		if (streq(what, k_info_flags2_rune[i]))
		{
			(*f2) |= (1L << i);
			return 0;
		}
...
/* Scan for the values */
		...
		if ((buf[0] == 'C') && (buf[2] == 'g'))
		{
			...
			if (streq(buf + 4+o, "All Gods"))
				mask = 0xFFFFFFFF;
			else
			{
				if ((i = find_god(buf + 4+o)) == -1) return (1);
				mask = BIT(i);
			}
			if(!o) c_ptr->gods |= mask;
			else   c_ptr->gods &= ~mask;
		...
		if ((buf[0] == 'C') && (buf[2] == 'a'))
		{
			...
			if (buf[4] == 'g')
			{
				...
				if (streq(buf + 6+o, "All Gods"))
					mask = 0xFFFFFFFF;
				else
				{
					if ((i = find_god(buf + 6+o)) == -1) return (1);
					mask = BIT(i);
				}
				if(!o) s_ptr->gods |= mask;
				else   s_ptr->gods &= ~mask;
	...
	while (0 == my_fgets_dostack(buf, 1024))
	{
		...
		if (buf[0] == 'F' || buf[0] == 'f')
		{
			...
			for (s = buf + 2; *s; )
			{
				...
				/* Parse this entry */
				if (0 != grab_one_kind_flag(k_ptr, s, buf[0] == 'f')) return (5);
	...
	while (0 == my_fgets_dostack(buf, 1024))
	{
		...
		if (buf[0] == 'F' || buf[0] == 'f')
		{
			...
			for (s = buf + 2; *s; )
			{
				...
				/* Parse this entry */
				if (0 != grab_one_artifact_flag(a_ptr, s, buf[0]=='f')) return (5);
...
static bool grab_one_ego_item_flag(ego_item_type *e_ptr, cptr what, int n, bool obvious)
{
	...
	for (i = 0; i < 32; i++)
	{
		if (streq(what, k_info_flags1[i]))
		{
			if (obvious) e_ptr->oflags1[n] |= (1L << i);
			e_ptr->flags1[n] |= (1L << i);
			return 0;
		}
	}
	...
	for (i = 0; i < 32; i++)
	{
		if (streq(what, k_info_flags2[i]))
		{
			if (obvious) e_ptr->oflags2[n] |= (1L << i);
			e_ptr->flags2[n] |= (1L << i);
			return 0;
		}
	}
	...
	for (i = 0; i < 32; i++)
	{
		if (streq(what, k_info_flags3[i]))
		{
			if (obvious) e_ptr->oflags3[n] |= (1L << i);
			e_ptr->flags3[n] |= (1L << i);
			return 0;
		}
	}
	...
	for (i = 0; i < 32; i++)
	{
		if (streq(what, k_info_flags4[i]))
		{
			if (obvious) e_ptr->oflags4[n] |= (1L << i);
			e_ptr->flags4[n] |= (1L << i);
			return 0;
		}
	}
	...
	for (i = 0; i < 32; i++)
	{
		if (streq(what, k_info_flags5[i]))
		{
			if (obvious) e_ptr->oflags5[n] |= (1L << i);
			e_ptr->flags5[n] |= (1L << i);
			return 0;
		}
	}
	...
	while (0 == my_fgets_dostack(buf, 1024))
	{
		...
		if (buf[0] == 'F' || buf[0] == 'f')
		{
			...
			for (s = buf + 2; *s; )
			{
				...
				/* Parse this entry */
				if (0 != grab_one_ego_item_flag(e_ptr, s, cur_r, buf[0]=='f')) return (5);
...
static errr grab_one_spell_flag(monster_race *r_ptr, cptr what)
{
	...
	/* Hack, should move to own flag set */
	for (i = 0; i < 32; i++)
	{
		if (streq(what, r_info_flags9[i]))
		{
...
/* Scan for the values */
		...
		if (buf[0] == 'S')
		{
			...
			for (s = buf + 2; *s; )
			{
				...
				if (2 == sscanf(s, "%d_IN_%d", &j, &i))
				{
					/* Extract a "frequency" */
					r_ptr->freq_spell = r_ptr->freq_inate = 100 * j / i;
	...
	while (0 == my_fgets_dostack(buf, 1024))
	{
		...
		if (buf[0] == 'S')
		{
			...
			for (s = buf + 2; *s; )
			{
				...
				{
					/* Extract a "frequency" */
					re_ptr->freq_spell = re_ptr->freq_inate = 100 * j / i;
...
#define SCAN_MON_FLAGS(X) \
	for(i = 0; i < 32; i++) \
		if(streq(what, r_info_flags##X[i])) \
		{ \
			d_ptr->rules[rule].mflags##X |= (1L << i); \
			return 0; \
		}
/*
 * Grab one (basic) flag in a monster_race from a textual string
