...
static void display_entry(int pos)
{
	...
	int maxwid = 75;
	int show_steal = get_skill(SKILL_STEALING) > 0;
	...
	else
	{
		...
		/* And for steal chance */
		if (show_steal) maxwid -= 4;
		...
		if (o_ptr->ident & (IDENT_FIXED))
		{
			...
			c_put_str(color, out_val, i + 6, cur_col);
			cur_col += 11;
		}
		...
		if (show_steal)
		{
			...
			int difficulty = 40-p_ptr->stat_ind[A_DEX] +
				o_ptr->weight/(5+get_skill_scale(SKILL_STEALING, 15)) -
				get_skill_scale(SKILL_STEALING, 15);
			int chance = difficulty > 10 ? 100*10/difficulty : 100;
			strnfmt(out_val, 160, "%3d", chance);
			c_put_str(TERM_BLUE, out_val, i + 6, cur_col);
			cur_col += 4;
		}
	}
}
...
void display_store(void)
{
	...
	else
	{
		...
		if (show_weights) maxwid -= 7;
		if (show_steal) maxwid -= 4;
		put_str("Item Description", 5, cur_col);
		cur_col += maxwid;

		/* If showing weights, show label */
		if (show_weights)
		{
			put_str("Weight", 5, cur_col-4);
			cur_col += 6;
		}

		/* Label the asking price (in stores) */
		put_str("Price", 5, cur_col);
		cur_col += 11;

		if(show_steal)
		{
			put_str("Stl%", 5, cur_col-5);
			cur_col += 11;
		}
	}
