...
{
	exec_lua(format("add_brand(%d,%d,%d,%d,%d)",rad, dam, gf, flag, v));
...
bool mon_take_hit(int m_idx, int dam, bool *fear, cptr note)
{
	...
	if (m_ptr->hp < 0)
	{
		...
		if (!note)
		{
			...
			u32b f1, f2, f3, f4, f5, esp;
			int i, j;
			for(i = 0, j = 0; i < BODY_MAX; i++)
			{
				/* Access the weapon */
				if(p_ptr->body_parts[i] != INVEN_WIELD) continue;
				o_ptr = &p_ptr->inventory[INVEN_WIELD+j++];
		...
		*/
		...
		/* Do this always */
		if (1 || m_ptr->ml || (r_ptr->flags1 & (RF1_UNIQUE)))
		{
			/* Count kills this life */
			if(d_ptr->flags2 & DF2_INVERT_DEATH && r_ptr->r_pkills > 0)
				r_ptr->r_pkills--;
			if(!(d_ptr->flags2 & DF2_INVERT_DEATH) && r_ptr->r_pkills < MAX_SHORT)
				r_ptr->r_pkills++;
	...
	while (!done)
	{
		...
		if (flag && temp_n)
		{
			...
			if (target_able(c_ptr->m_idx))
				snprintf(info, sizeof(info), "q,t,p,o,+,-,'dir' (%d,%d)", x, y);
			/* Dis-allow target */
			else
				snprintf(info, sizeof(info), "q,p,o,+,-,'dir' (%d,%d)", x, y);
		...
		else
		{
			...
			/* Default prompt */
			snprintf(info, sizeof(info), "q,t,p,m,+,-,'dir' (%d,%d)", x, y);
...
bool get_aim_xy(int * eflg, int * x, int * y)
{
	int dir = 0;
	*eflg = 0;
	if(!get_aim_dir(&dir)) return FALSE;
	/* Use the given direction */
	*x = p_ptr->px + 99 * ddx[dir];
	*y = p_ptr->py + 99 * ddy[dir];
		...
		if (strstr(mname, buf))
		{
			...
			for (j = max_re_idx - 1; j >= 0; j--)
			{
				...
				if (!stricmp(mname, buf))
				{
					...
					egos[0] = j;
					/* Create the monster */
					if (place_monster_one(wy, wx, i, egos, FALSE, mstatus))
						msg_print("Your wish becomes truth!");
