 ...
 */
static void roff_aux(int r_idx, u16b * ego, int remem)
{
	monster_race *r_ptr;
 ...
 */
static void roff_name(int r_idx, u16b * ego)
{
	monster_race *r_ptr = race_info_idx(r_idx, ego);

	byte	a1, a2;
	char	c1, c2;
	char    nbuf[0x200], pbuf[0x200];
	int     i;
	...
	/* Dump the name */
	sprintf(nbuf, "%s", r_name + r_ptr->name);
	for(i = 0; i < MEGO_MAX_STACK && ego[i]; i++)
	{
		if(re_info[ego[i]].before)
			sprintf(pbuf, "%s %s", re_name + re_info[ego[i]].name, nbuf);
		else
			sprintf(pbuf, "%s %s", nbuf, re_name + re_info[ego[i]].name);
		strcpy(nbuf, pbuf);
	}

	Term_addstr(-1, TERM_WHITE, nbuf);
 ...
 */
static void roff_top(int r_idx, u16b * ego)
{
	/* Clear the top line */
	Term_erase(0, 0, 255);
 ...
 */
void screen_roff(int r_idx, u16b * ego, int remember)
{
	/* Flush messages */
	msg_print(NULL);
 ...
 */
void monster_description_out(int r_idx, u16b * ego)
{
	roff_name(r_idx, ego);
 ...
 */
void display_roff(int r_idx, u16b * ego)
{
	int y;
