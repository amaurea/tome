...
static cptr likert(int x, int y)
{
	...
	snprintf(dummy, sizeof(dummy), "%d", x);
...
static void display_player_various(void)
{
	...
	if (p_ptr->melee_style == SKILL_HAND || p_ptr->melee_style == SKILL_BEAR)
	{
		...
		i = max_blow - 1;
		while (blow_table[i].min_level > plev && i != 0) --i;
	...
	{
		int from = 0, to = 0, i, tmp, obonus;
		char buf[0x100], tbuf[0x100], dstr[0x100];
		...
		for(i = 0; p_ptr->body_parts[i] == INVEN_WIELD; i++)
		{
			...
			obonus = object_known_p(o_ptr) ? o_ptr->to_d : 0;
			damdice = o_ptr->dd;
			damsides = o_ptr->ds;

			tmp = (damdice + dambonus + obonus)*p_ptr->num_blow_limb[i];
			from += tmp > 0 ? tmp : 0;

			tmp = (damdice*damsides + dambonus + obonus)*p_ptr->num_blow_limb[i];
			to += tmp > 0 ? tmp : 0;

			snprintf(dstr, sizeof(dstr), "%d", p_ptr->num_blow_limb[i]);
			strncpy(tbuf, buf, sizeof(tbuf));
			snprintf(buf, sizeof(buf), "%s%s%s",tbuf, i?"+":"", dstr);
		}

		if(to == 0) desc = "nil!";
		else if(from == to) desc = format("%d", to);
		else desc = format("%d-%d", from, to);
...
void expand_flags(u32b f1, u32b f2, u32b f3, u32b f4, u32b f5, u32b esp, int mul,
	int * of1, int * of2, int * of3, int * of4, int * of5, int * oesp)
{
	int j;
	for(j=0; j< 32; j++)
	{
		of1[j]  = f1&(1<<j)  ? TR1_PVAL & (1<<j) ? mul : 1<<16 : 0;
		of2[j]  = f2&(1<<j)  ? TR2_PVAL & (1<<j) ? mul : 1<<16 : 0;
		of3[j]  = f3&(1<<j)  ? TR3_PVAL & (1<<j) ? mul : 1<<16 : 0;
		of4[j]  = f4&(1<<j)  ? TR4_PVAL & (1<<j) ? mul : 1<<16 : 0;
		of5[j]  = f5&(1<<j)  ? TR5_PVAL & (1<<j) ? mul : 1<<16 : 0;
		oesp[j] = esp&(1<<j) ? ESP_PVAL & (1<<j) ? mul : 1<<16 : 0;
	}
}
...
{
	int i, j, k, n, x, y, z, dispx, modetemp, xtemp;
	...
	int b[INVEN_TOTAL - INVEN_WIELD + 1][6][32];
	...
	for(i=0;i<sizeof(b)/sizeof(int);i++) ((int*)&b[0][0][0])[i] = 0;
	...
	for (i = INVEN_WIELD; i < INVEN_TOTAL; i++)
	{
		...
		expand_flags(f1,f2,f3,f4,f5,esp,o_ptr->pval,&b[n][0][0],&b[n][1][0],&b[n][2][0],&b[n][3][0],&b[n][4][0],&b[n][5][0]);
	}
	...
	/* Player flags = total flags - all object contributions */
	get_effective_flags(&b[n][0][0],&b[n][1][0],&b[n][2][0],&b[n][3][0],&b[n][4][0],&b[n][5][0]);
	for(i = 0; i < n; i++)
	for(j = 0; j < 6; j++)
	for(k = 0; k < 32; k++)
		b[n][j][k] -= b[i][j][k];
	/* Make sure that we don't have any negative flags left */
	for(j = 0; j < 6; j++)
	for(k = 0; k < 32; k++)
		if(abs(b[n][j][k])&0xffff0000 && b[n][j][k] < 0)
			b[n][j][k] = 0;
			...
			for (z = mode; z <= modetemp; z++)
			{
				...
				for (n = 0; n < INVEN_TOTAL - INVEN_WIELD + 1; n++)
				{
					...
					if (i)
					{
						...
						/* Non-flags are encoded in the lower 16 bits */
						else if (!(abs(i) & 0xffff0000))
						{
							c = i;
							if (c < 0)
							{
...
errr file_character(cptr name, bool full)
{
	...
	if (joke_monsters)
		...
		fprintf(fff, "\n Joke monsters:        OFF");
	if (zang_monsters)
		fprintf(fff, "\n Zangband monsters:    ON");
	else
		fprintf(fff, "\n Zangband monsters:    OFF");
	if (cth_monsters)
		fprintf(fff, "\n Lovecraft monsters:   ON");
	else
