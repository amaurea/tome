...
struct object_kind
{
	...
	s32b chance[4];		/* Allocation chance(s) */
...
#endif
	s16b effects[MAX_EFFECTS_TILE];            /* The lasting effects */
};
...
struct effect_type
{
	...
	s16b    who;            /* Who owns this effect */
	s16b    speed;       /* How often */
	s16b    energy;
	u32b    flags;          /* Flags */
};
...
struct monster_type
{
	...
	u16b ego[MEGO_MAX_STACK]; /* Ego monster type */
...
struct player_type
{
	...
	s16b num_blow_limb[32]; /* Blows per limb */
...
struct skill_type
{
	...
	s32b raw;                               /* Raw value - value before class mods */
