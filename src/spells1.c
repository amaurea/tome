...
/* Returns true with probability val:odds */
bool test_odds(int val, int odds)
{
	return randint(val + odds) > odds;
}
...
byte spell_color(int type)
{
	...
	else if (use_color)
	{
		...
		case GF_UNLIGHT:
			return (randint(2) == 1 ? TERM_DARK : (randint(2) == 1 ?
				TERM_L_DARK : TERM_VIOLET));
		case GF_PLASMA:
...
byte get_gf_color(int type) { return spell_color(type); }

/* This function uses static storage to store the name it
 * returns a pointer to. This means that the pointer will only
 * stay valid until the next call of this function. */
char * get_gf_name(int gf)
{
	int i;
	static char buffer[0x100];
	if (process_hooks_ret(HOOK_GF_NAME, "s", "(d)", gf))
	{
		if(strlen(process_hooks_return[0].str) >= 0x100) return 0;
		strncpy(buffer, process_hooks_return[0].str, 0x100);
		return buffer;
	}
	else
	{
		for(i = 0; gf_names[i].gf != -1 && gf_names[i].gf != gf; i++);
		if(gf_names[i].gf != -1) return gf_names[i].name;
	}
	return 0;
}
...
/*
 * Applies damage to player. Takes various shields etc. into
 * account, and calls modify_hp etc. as needed.
 */
void take_hit(int damage, cptr hit_from)
{
	int sdam, spectre = PRACE_FLAG(PR1_SEMI_WRAITH), mul = 4, div = 2;
	...
	damage -= symbiote_shield(damage);

	/* Apply disruption shield. Do so after the carried monster to make */
	/* symbiosis useful for people who rely on the shield. Spectres have */
	/* inherent disruption shield */
	if (p_ptr->disrupt_shield || spectre)
	{
		sdam = p_ptr->csp > damage*mul/div ? damage*mul/div : p_ptr->csp;
		modify_sp(-sdam);
		damage -= sdam*div/mul;
	}

	modify_hp(-damage, hit_from);
}
...
void take_sanity_hit(int damage, cptr hit_from)
{
	...
	/* Hitpoint warning */
	if (alert_hitpoint && p_ptr->csane < warning)
	{
		/* Hack -- bell on first notice */
		if (old_csane > warning) bell();
...
void set_hp(int value, cptr hit_from) { modify_hp(value-p_ptr->chp, hit_from); }
void set_sp(int value) { modify_sp(value-p_ptr->csp); }

// Add amount to current hp, capping if necessary, and
// handling death etc. Not the same as take_hit, as
// take hit takes various shields etc. into account.
void modify_hp(int value, cptr hit_from)
{
	char death_message[80];
	int warning, percent, old;
	...
	if (p_ptr->chp < 0)
	{
		...
		if (((!has_ability(AB_UNDEAD_FORM)) || ((p_ptr->necro_extra & CLASS_UNDEAD))))
		{
			...
			/* Note death */
			death = TRUE;
			if (get_check("Dump the screen? ")) do_cmd_html_dump();
	...
	/* Hitpoint warning */
	warning = (p_ptr->mhp * hitpoint_warn / 10);
	if (!p_ptr->wild_mode && alert_hitpoint && p_ptr->chp < warning)
	{
		/* Hack -- bell on first notice */
		if (old > warning) bell();
		sound(SOUND_WARN);
	...
	if(!p_ptr->wild_mode) {
		process_hooks(HOOK_HP_CHANGED, "()");
		/* Melkor can summon to help you */
		melkor_help(percent);
	}
...
// Directly modify sp, without any redirection, but with the
// consequences, as modify_hp above.
void modify_sp(int value)
{
	int warning, spectre, percent, old;
	spectre = PRACE_FLAG(PR1_SEMI_WRAITH);

	/* Hurt the player */
	old = p_ptr->csp;
	p_ptr->csp += value;
	if(value > 0 && old > 0 && p_ptr->csp < 0) p_ptr->csp = MAX_SHORT;
	...
	/* Sp warning, only for spectres */
	warning = (p_ptr->msp * hitpoint_warn / 10);
	if(!p_ptr->wild_mode && alert_hitpoint && spectre && p_ptr->csp < warning)
	{
		/* Hack -- bell on first notice */
		if (old > warning) bell();
		sound(SOUND_WARN);
		cmsg_print(TERM_RED, "*** LOW SP WARNING! ***");
		msg_print(NULL);
	}
	...
	percent = p_ptr->chp * 100 / p_ptr->mhp;
	if(!p_ptr->wild_mode) {
		process_hooks(HOOK_SP_CHANGED, "()");
		/* Melkor can summon to help you */
		if(spectre) melkor_help(percent);
	}

	if(spectre && player_char_health) lite_spot(p_ptr->py, p_ptr->px);
}

void melkor_help(int percent)
{
	...
	PRAY_GOD(GOD_MELKOR)
	{
		...
		if (magik(chance - 10))
		{
			...
			if (magik(50)) type = SUMMON_UNDEAD;
...
// Give the symbiote a chance to shield, and return the amount shielded.
int symbiote_shield(int damage)
{
	int old_dam = damage;
	...
	if (o_ptr->k_idx && (magik(5 + get_skill(SKILL_SYMBIOTIC))) && (!carried_monster_hit))
	{
		...
		{
			cmsg_format(TERM_L_RED, "%s dies from protecting you, you feel very sad...", sym_name);
...
/* This is used by the unlight element. A meaner, hungrier version
 * of disenchant. Returns nonzero if disenchantable. Provides the energy drained
 * and the potential before draining. */
int star_disenchant_item(object_type * o_ptr, int power, int * drained, int * potential,
	int * kill)
{
	int tmp, is_art;
	int pot  = 0, drain= 0;
	...
	*kill      = 0;
	...
	is_art = artifact_p(o_ptr) || o_ptr->art_name;
	if(is_art) power /= 5;
	if(f5 & TR5_INSULATE) power /= 5;

	pot += 50*o_ptr->to_h;
	if(o_ptr->to_h > 0 && test_odds(power, base))
	{
		tmp = randint(1+power*2/base);
		o_ptr->to_h -= tmp;
		drain   += 50*tmp;
	}
	pot += 50*o_ptr->to_d;
	if(o_ptr->to_d > 0 && test_odds(power, base))
	{
		tmp = randint(1+power*2/base);
		o_ptr->to_d -= tmp;
		drain   += 50*tmp;
	...
	pot += 50*o_ptr->to_a;
	if(o_ptr->to_a > 0 && test_odds(power, base))
	{
		tmp = randint(1+power*2/base);
		o_ptr->to_a -= tmp;
		drain   += 50*tmp;
	...
	if(o_ptr->tval == TV_ROD)
	{
		...
		{
			pot += 5*o_ptr->pval2;
			if(test_odds(power, base/5))
			{
				tmp = o_ptr->pval2;
				o_ptr->pval2 -= randint(1+power*30/base);
				if(o_ptr->pval2 < 0) o_ptr->pval2 = 0;
				drain += 5*(tmp-o_ptr->pval2);
			}
		}
	}
	else if(o_ptr->tval == TV_STAFF || o_ptr->tval == TV_WAND)
	{
		if(o_ptr->pval > 0)
		{
			pot += 20*o_ptr->pval;
			if(test_odds(power, base/2))
			{
				tmp = o_ptr->pval;
				o_ptr->pval -= randint(1+power*4/base);
				if(o_ptr->pval < 0) o_ptr->pval = 0;
				drain += 20*(tmp-o_ptr->pval);
			}
		}
		if(o_ptr->sval != SV_WAND_NOTHING)
		{
			pot += 50;
			if(o_ptr->pval == 0 && test_odds(power, base))
			{
				o_ptr->sval = SV_WAND_NOTHING;
				drain += 50;
			}
		}
	}
	/* yes, this one is evil: Drain pvals!! */
	else if(o_ptr->pval > 0)
	{
		pot += 500*o_ptr->pval;
		if(test_odds(power, base*20))
		{
			tmp = randint(1+power/4/base);
			o_ptr->pval -= tmp;
			drain += tmp*500;
		}
	}
	...
	/* Should ideally drain flags, but this seems to be
	 * impossible for non-randarts. */

	/* Should we destroy the whole item? Use gold value to
	 * resist. This doesn't make much sense on face value, but it
	 * is used as a proxy for how much magic is inherently in the object..*/
	if(!is_art) {
		int residual = (object_value_real(o_ptr)/100 + o_ptr->weight)*base/100;
		pot += residual;
		if(test_odds(power, residual))
		{
			*kill = randint(power/base+1);
			drain += residual;
		}
	}

	if(potential) *potential = pot;
	if(drained)   *drained   = drain;
...
sint project_path(u16b *gp, int range, int y1, int x1, int y2, int x2, int flg)
{
	...
	if (flg & PROJECT_MANA_PATH)
	{
		...
		while (1)
		{
			...
			if (cave_perma_grid(&cave[y][x])) break;
...
static bool project_f(int who, int r, int y, int x, int dam, int typ)
{
	...
	case GF_UNLIGHT:
		{
			int base = 150;
			int bleh = 1, ok = bleh*5, yum = ok*5;
			tmp = 0;
			/* Eats light */
			if(c_ptr->info & (CAVE_GLOW|CAVE_PLIT|CAVE_MLIT)) { c_ptr->info &= ~CAVE_GLOW; tmp += yum; }
			if(cave_plain_floor_grid(c_ptr) && !(c_ptr->info & (CAVE_TRDT)))
				c_ptr->info &= ~CAVE_MARK;
			/* Eats mana */
			tmp += c_ptr->mana;
			c_ptr->mana = 0;

			/* Tries to "eat" everything, making it move closer
			 * to plain, empty floor */
			...
			case FEAT_WALL_SOLID:
				if(!test_odds(dam, base*8)) break;
				cave_set_feat(y,x,FEAT_FLOOR);
				tmp += bleh;
			case FEAT_EMPTY_FOUNTAIN:
				if(!test_odds(dam, base*4)) break;
				cave_set_feat(y,x,FEAT_FLOOR);
				tmp += bleh;
				break;
			case FEAT_FOUNTAIN:
				if(!test_odds(dam, base/2)) break;
			...
			case FEAT_SMALL_TREES:
				if(!test_odds(dam, base)/6) break;
				cave_set_feat(y,x,FEAT_DEAD_SMALL_TREE);
				tmp += yum;
				break;
			case FEAT_DEAD_TREE:
			case FEAT_DEAD_SMALL_TREE:
				if(!test_odds(dam, base)) break;
			...
			else if(m_list[who].r_idx)
			{
				monster_gain_exp(who, 10*tmp, TRUE);
				m_list[who].hp += tmp;
				if(m_list[who].hp > m_list[who].maxhp)
					m_list[who].hp = m_list[who].maxhp;
			}
			break;
		}
	...
	case GF_WEB:
		{
			...
			if ((f_info[c_ptr->feat].flags1 & FF1_PERMANENT)) break;
			/* Create a closed door */
			if(magik(200/(r+4))) cave_set_feat(y, x, FEAT_WEB);
			/* Observe */
			if (c_ptr->info & (CAVE_MARK)) obvious = TRUE;
			/* Update some things */
			p_ptr->update |= (PU_VIEW | PU_MONSTERS | PU_MON_LITE);
...
static bool project_o(int who, int r, int y, int x, int dam, int typ)
{
	...
	for (this_o_idx = c_ptr->o_idx; this_o_idx; this_o_idx = next_o_idx)
	{
		...
		case GF_RAISE:
			{
				...
				if (o_ptr->tval == TV_CORPSE)
				{
					u16b ego[2] = { raise_ego[rand_int(MAX_RAISE)], 0 };
		...
		case GF_UNLIGHT:
			{
				int drain,kill=0;
				if(star_disenchant_item(o_ptr, dam, &drain, NULL, &kill));
				{
					do_kill = kill;
					if(kill) note_kill = plural ? " crumble to dust!" : " crumbles to dust!";
					if(o_ptr->marked)
					{
						char o_name[80];
						object_desc(o_name, o_ptr, FALSE, 3);
						if(drain)
							msg_format("The %s %s eroded!",
								o_name,((o_ptr->number != 1) ? "were" : "was"));
						/*else
							msg_format("The %s resist%s disenchantment!",
...
bool project_m(int who, int r, int y, int x, int dam, int typ)
{
	...
	monster_type *m_ptr = &m_list[c_ptr->m_idx], *caster_ptr;
	...
	case GF_UNBREATH:
		...
		/* Ungoliant's unlight. Drains. Should also weaken monsters. */
	case GF_UNLIGHT:
		{
			obvious = do_monster_unlight(&dam, c_ptr->m_idx, who, seen, &note);
			break;
		}
	...
	case GF_OLD_DRAIN:
		{
			...
			{
				/* Heal no more than the targets hp */
				heal = (dam < m_ptr->hp) ? dam : m_ptr->hp;
				if(who){
					caster_ptr = &m_list[who];
					/* Heal */
					caster_ptr->hp += heal;
					...
					if(health_who == who) p_ptr->redraw |= (PR_HEALTH);
				}
				else hp_player(heal);
			}
		...
		/* Drain Life with high Necromancy skill */
	case GF_UNHOLY_DRAIN:
		{
			if (seen) obvious = TRUE;
	...
	default:
		{
			...
			{
				obvious = seen && process_hooks_return[0].num;
	                ...
	                !(r_ptr->flags4 & (RF4_BR_WALL)) && hurt_monster(m_ptr))
	{
		...
		if (m_ptr->stunned)
		{
			...
			if (m_ptr->stunned < 2*do_stun)
				tmp = tmp > 2*do_stun ? 2*do_stun : tmp;
		}
		else
		{
			note = " is dazed.";
...
static bool project_p(int who, int r, int y, int x, int dam, int typ, int a_rad)
{
	...
	case GF_WEB:
		{
			if (fuzzy) msg_print("You are hit by sticky webs!");
			(void)set_slow(p_ptr->slow + rand_int(4) + 4);
			take_hit(dam, killer);
			break;
		}
	...
	case GF_UNLIGHT: { do_player_unlight(dam, who, fuzzy); break; }
		/* Default */
	default:
		{
			/* Hooks! */
 ...
 */
 ...
 /* Not sure where to put this ugly hack, so I'll put it here */
int interval_project_hack = 10;
...
bool project(int who, int rad, int y, int x, int dam, int typ, int flg)
{
	...
	if (flg & (PROJECT_GRID))
	{
		...
		if (flg & PROJECT_STAY)
		{
			effect = new_effect(typ, dam, project_time, p_ptr->py, p_ptr->px, rad, who, interval_project_hack, project_time_effect);
			project_time = 0;
		...
		for (i = 0; i < grids; i++)
		{
			...
			if (flg & PROJECT_STAY)
			{
				add_effect_tile(&cave[y][x], effect);
				lite_spot(y, x);
			}
		}
	}
	...
	if (flg & (PROJECT_KILL))
	{
		...
		for (i = 0; i < grids; i++)
		{
			...
			/* Affect the player */
			/* Hack: Cloud effects effectively have rad = 0 here.
			 * This means that they are counted as bolt attacks,
			 * which does not make sense. This should be fixed properly,
			 * but for now I will get around it by checking if something has
			 * project_jump, and set rad to 1. Rad being 1 does not actually
			 * have any effect in project_p, other than disabling bounces.
			 */
			if(flg & PROJECT_JUMP) rad = 1;
...
void do_player_unlight(int dam, int who, int fuzzy)
{
	char killer[80];
	int denom = 3, i, idx = -1, kill, drain = 0;
	/* Light provides resistance. Darkness resistance also helps a bit. */
	int res = 1+p_ptr->cur_lite + p_ptr->resist_dark;
	int heal = dam;
	if(who < 0) strcpy(killer, "your symbiote");
	else monster_desc(killer, &m_list[who], 0x88);
	object_type * o_ptr;
	dam = dam*denom/res;
	if(fuzzy) msg_print("You are hit by pure devouring!");
	take_hit(dam, killer);
	if(!p_ptr->hold_life || magik(25)) lose_exp(dam*50);
	/* Try to *disenchant* equipment.*/
	idx = randint(INVEN_TOTAL)-1;
	o_ptr = &p_ptr->inventory[idx];
	if(o_ptr->k_idx)
	{
		if(star_disenchant_item(o_ptr, p_ptr->resist_disen ? dam/3 : dam, &drain, NULL, &kill))
		{
			char o_name[80];
			object_desc(o_name, o_ptr, FALSE, 3);
			if(kill)
			{
				msg_format("%d of your %s (%c) crumble%s to dust!",
					kill, o_name, index_to_label(idx),((o_ptr->number != 1) ? "" : "s"));
				inven_item_increase(idx, -kill);
				inven_item_optimize(idx);
			}
			else if(drain)
				msg_format("Your %s (%c) %s disenchanted!",
					o_name, index_to_label(idx),((o_ptr->number != 1) ? "were" : "was"));
		}
	}
	/* The attacker is healed based on damage. The healing is
	 * amplified in the presence of light (the light was eaten) */
	heal *= (1+p_ptr->cur_lite)/denom + drain;
	if (who == 0)
	{
		gain_exp(10*heal);
		hp_player(heal/5);
	}
	else if(who > 0 && m_list[who].r_idx)
	{
		monster_gain_exp(who, 10*heal, TRUE);
		m_list[who].hp += heal/5;
		if(m_list[who].hp > m_list[who].maxhp)
			m_list[who].hp = m_list[who].maxhp;
	}
}

int do_monster_unlight(int * dam, int m_idx, int who, int seen, char ** note)
{
	...
	/* Weaken monster ala melkor curse */
	exec_lua(format("do_melkor_curse(%d,%d,%d)", m_idx, randint(*dam/3),seen));
	...
	{
		monster_gain_exp(who, *dam*10, TRUE);
		m_list[who].hp += *dam/20;
		if(m_list[who].hp > m_list[who].maxhp)
			m_list[who].hp = m_list[who].maxhp;
	}
	return seen;
