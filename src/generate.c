 ...
 * the pit will be empty, and will not affect the level rating.
 *
...
void generate_cave(void)
{
	...
	/* Clear all non-transferable effects. Only player-centered
	 * storm effects make sense to transfer */
	for(i = 0; i < MAX_EFFECTS; i++)
		if(!(effects[i].who == 0 && effects[i].flags & EFF_STORM))
			effects[i].time = 0;
	...
	if (get_dungeon_save(buf))
	{
		...
		for (y = 0; y < MAX_HGT; y++)
		{
			for (x = 0; x < MAX_WID; x++)
			{
				...
				/* No effects */
				clear_all_effects_tile(&cave[y][x]);
