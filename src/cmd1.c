 ...
 *
 * Factor in weapon weight, total plusses and skills. Made criticals rarer for
 * normal players, since criticals have become more powerful. A high-level
 * character with a typical weapon can now expect a critical about 5% of the time,
 * while one with a maxed critical hits skill will be around 25%.
 *
 */
s16b critical_norm(int weight, int plus, int dam, int weapon_tval, bool *done_crit)
{
	int i, num = randint(5000), do_crit = FALSE;
	int power = 0;
	...
	if (p_ptr->tim_deadly)
	{
		...
		do_crit = TRUE;
		// Guaranteed at least a superb hit
		power   = 75;
	}
	else
	{
		/* Extract "blow" power */
		i = (weight + (p_ptr->to_h + plus) * 5/2 +
		     get_skill_scale(p_ptr->melee_style, 75));
		i += 100 * p_ptr->xtra_crit;
		if ((weapon_tval == TV_SWORD) && (weight < 50))
			i += get_skill_scale(SKILL_CRITS, 1000);
		i += luck(-100, 100);

		if(num < i)
		{
			do_crit = TRUE;
			// Possible, but rare, to get very good criticals
			do power += randint(50); while(!randint(5));
		}
	}
	...
	if(do_crit)
	{
		...
		if (weapon_tval == TV_SWORD && weight < 50 && get_skill(SKILL_CRITS))
			power += get_skill_scale(SKILL_CRITS, 125);

		/* With a light weapon, you can expect about 15% extra damage, 20% with luck.
		 * With a heavy one, expect about 20% extra, 30% with luck.
		 * With deadly hits, you get 35% extra for small weapons and 50% extra for
		 * heavy weapons.
		 * With a high critical skill, expect 50-100% extra damage.
		 * With both deadly hits and critical skill, you get 200% extra damage.
		 */
		if     (power < 25) { dam+= 16*(dam+16)/128;msg_print("It was a fine hit!"); }
		else if(power < 50) { dam+= 23*(dam+16)/128;msg_print("It was a good hit!"); }
		else if(power < 75) { dam+= 32*(dam+16)/128;msg_print("It was a great hit!"); }
		else if(power <100) { dam+= 45*(dam+16)/128;msg_print("It was a superb hit!"); }
		else if(power <125) { dam+= 64*(dam+16)/128;msg_print("It was a *FINE* hit!"); }
		else if(power <150) { dam+= 91*(dam+16)/128;msg_print("It was a *GOOD* hit!"); }
		else if(power <175) { dam+=128*(dam+16)/128;msg_print("It was a *GREAT* hit!"); }
		else if(power <200) { dam+=181*(dam+16)/128;msg_print("It was a *SUPERB* hit!"); }
		else                { dam+=256*(dam+16)/128;msg_print("It was a *PERFECT* hit!");}
	}
	*done_crit = do_crit;
	return dam;
}
...
void search(void)
{
	...
	power = p_ptr->skill_srh;
	if(power < 0) power = 0;

	/* Penalize various conditions */
	if (p_ptr->blind || no_lite()) power /= 5;
	if (p_ptr->confused || p_ptr->image) power /= 5;

	/* The player searches in ± this distance from the
	 * current location. This should ideally be  at 50 in
	 * the searching skill. I will assume that skll_srh
	 * is simply skill(50) + item(25) + bonus(10),
	 * where item indicates the largest bonus you could get
	 * from items if you tried to optimize for searching.
	 * The formula used here is simply division with up-rounding. */
	rad  = power ? (power+10-1)/10 : 0;
	ntry = power ? (power+40-1)/40 : 0;
	...
	for (y = p_ptr->py - rad; y <= p_ptr->py + rad; y++)
	{
		for (x = p_ptr->px - rad; x <= p_ptr->px + rad; x++)
		{
			...
			/* Sometimes, notice things. The odds are 2*power:difficulty
			 * per test, where difficulty is how well-hidden the trap is,
			 * which is taken to be the dungeon level for simplicity.
			 * At high levels one also gains bonus attempts to find
			 * traps, making the search skill very reliable at high levels,
			 * but not so much at low levels.
			 */
			for(i=nok=0; i < ntry; i++) nok += rand_int(dun_level+2*power) > dun_level;
			if (nok)
			{
				/* Access the grid */
				c_ptr = &cave[y][x];
				...
				/* If our skill is high enough, mark the square as trap-detected */
				if(power > 35) c_ptr->info |= CAVE_DETECT;
			}
		}
	}
}
...
static void py_attack_hand(int *k, monster_type *m_ptr, s32b *special)
{
	...
	if (r_ptr->flags3 & (RF3_UNDEAD|RF3_NONLIVING)) resist_stun += 88;
	...
	*k = critical_norm(plev * (randint(10)), ma_ptr->min_level, *k, -1, &done_crit);
	...
	if (stun_effect && *k < m_ptr->hp)
	{
		if (plev > randint(m_ptr->level + resist_stun + 10))
		{
			...
			/* It seems that this needed to be capped */
			tmp = m_ptr->stunned;
			m_ptr->stunned += stun_effect;
			if(m_ptr->stunned > 2*stun_effect)
				m_ptr->stunned = MAX(tmp,2*stun_effect);
		}
	}
}
 ...
 */
int do_nazgul(int *k, int weap, monster_race *r_ptr,
               object_type *o_ptr)
{
	u32b f1, f2, f3, f4, f5, esp;
...
{
	int num = 0, num_tot, num_default, num_cum, j, k, bonus, chance;
	s32b special = 0;
	...
	{
		msg_format("%^s is immune to melee attacks.", m_name);
		return;
	}
	...
	/* Stop if friendly */
	if ((is_friend(m_ptr) >= 0) && !(p_ptr->stun || p_ptr->confused || p_ptr->image ||
	!(m_ptr->ml)))
	{
	...
	else
	{
		...
		{
			weapons = 1;
			num_default = p_ptr->num_blow_limb[0];
		}

		if(max_blow < 0) max_blow = num_default;
		/* Calculate the rescaled number of blows for each weapon */
		for(i = 0, j = 0; i < weapons; i++, j+=num_default ? p_ptr->num_blow_limb[i] : 1)
			rescaled_blows[i] = num_default ?
				(j+p_ptr->num_blow_limb[i])*max_blow/num_default-j*max_blow/num_default :
				(j+1)*max_blow/weapons - j*max_blow/weapons;
		num_tot = 0;
		...
		for (weap = 0; weap < weapons; ++weap)
		{
			...
			int num_blow = rescaled_blows[weap];
			...
			else
			{
				...
				for(num = 0; num<num_blow; ++num, ++num_tot)
				{
					/* Test for hit */
					if (test_hit_norm(chance, m_ptr->ac, m_ptr->ml))
					{
						...
						else /* if (o_ptr->k_idx) */
						{
							...
							k += p_ptr->to_d + p_ptr->to_d_melee + o_ptr->to_d;

							k = tot_dam_aux(o_ptr, k, m_ptr, &special);

							if (backstab)
							{
								k += (k * get_skill_scale(SKILL_BACKSTAB, 300)) / 100;
								/* Was originally at 100)) */
							}
							if (stab_fleeing)
							{
								k += (k * get_skill_scale(SKILL_BACKSTAB, 210)) /
								     100;
								/* Was originally at 70)) */
							}
							...
							PRAY_GOD(GOD_TULKAS)
							{
								...
								else k += p_ptr->to_d_melee;
							}

							/* Project some more nasty stuff? */
							exec_lua(format("project_brand(%d,%d)",x,y));
							if (!c_ptr->m_idx)
							{
								mdeath = TRUE;
								break;
							...
							if(do_nazgul(&k, weap, r_ptr, o_ptr)) break;
						}
						...
						/* Message. Comes a bit too late. The backstab and
						 * crit messages come before you are informed about
						 * the hit! But we only know the final damage when
						 * we come here. */
						...
						else if (backstab)
						{
							...
							backstab = FALSE;
							if(stab_fleeing)
								msg_format("You cruelly stab the helpless, sleeping, fleeing %s!", buf);
							else msg_format("You cruelly stab the helpless, sleeping %s!", buf);
						}
						else
						{
							char buf[80];
						...
						if (mon_take_hit(c_ptr->m_idx, k, &fear, NULL))
						{
							 ...
							 * among weaker foes.
							 */
							if ((has_ability(AB_SPREAD_BLOWS)) && (num_tot < max_blow) && energy_use)
								energy_use = energy_use * num_tot / max_blow;
							mdeath = TRUE;
							break;
						}
...
void do_cmd_pet(void)
{
	...
	{
		strcpy(power_desc[num], "dismiss");
		powers[num++] = 1;
		strcpy(power_desc[num], "call");
		powers[num++] = 2;
		...
		powers[num++] = 5;
		strcpy(power_desc[num], "give target to friend(s)");
		powers[num++] = 8;
		strcpy(power_desc[num], "friend(s) forget target");
		powers[num++] = 9;
	}
	...
	case 9:
		{
			...
			chosen = display_allies(pets);
			if(chosen < 0){
				msg_print("This is not a pet/companion.");
				return;
			}
			if(chosen == 4)
			{
				...
				if (cave[jj][ii].m_idx)
				{
					m_idx = cave[jj][ii].m_idx;

					if (m_list[m_idx].status < MSTATUS_PET)
					{
			...
			if(chosen == 0)
			{
				for(j = 0; j < max_m_idx; j++)
					if(m_list[j].status >= MSTATUS_FRIEND)
						m_list[j].target = -1;
			}
			else if(chosen <= 3)
			{
				/* Friendlies*/
				if(chosen <= 1) status = MSTATUS_FRIEND;
				/* Pets */
				else if(chosen == 2) status = MSTATUS_PET;
				/* Companions */
				else status = MSTATUS_COMPANION;

				for(j = 0; j < max_m_idx; j++)
					if(m_list[j].status == status)
						m_list[j].target = -1;
			}
			/* Single ally */
			else
			{
				m_list[chosen - 5].target = -1;
			}
	...
	case 8:
		{
			...
			{
				m_list[chosen - 5].target = m_idx_foe;
			}
	...
	case 1:
		{
			...
			{
				if(dismiss_pet(chosen -5)) dismissed++;
			}
	...
	case 5:
		{
			...
			if (!p_ptr->pet_pickup_items)
			{
				...
				if(chosen == 0)
				{
					for(j = 0; j < max_m_idx; j++)
						if(m_list[j].status >= MSTATUS_FRIEND)
							monster_drop_carried_objects(&m_list[j]);
				}
				else if(chosen <= 3)
				{
					/* Friendlies*/
				...
				{
					monster_drop_carried_objects(&m_list[chosen - 5]);
				}
			}
...
int display_allies(int pets)
{
	int page, npage, nper = 20, i, chosen, counter = 0;
	char ch;
	Term_save();
	while(1)
	{
		Term_save();
		int j, o = 1, col;
		npage = (pets+nper-1)/nper;

		c_prt(TERM_WHITE, format("   %4s %-24.24s %3s %6s",
			"id", "name", "lvl", "%hp"), o++, 0);
		for(i = 0, j = -page*nper; i < max_m_idx && j < nper; i++)
		{
			if(m_list[i].status < MSTATUS_FRIEND) continue;
			if(m_list[i].r_idx == 0) continue;
			if(j++ < 0) continue;
			if(m_list[i].status == MSTATUS_PET) col = TERM_L_BLUE;
			else if(m_list[i].status == MSTATUS_COMPANION) col = TERM_YELLOW;
			else col = TERM_WHITE;
			char desc[0x200];
			monster_desc(desc, &m_list[i], 0x188);
			c_prt(col, format("%c) %4d %-24.24s %3d %6d",
				'a'+j-1 ,i, desc, m_list[i].level,
				100 * m_list[i].hp / m_list[i].maxhp), o++, 0);
		}
		if (!get_com(format("Pets (page %d of %d). Press All, Friendlies, Pets, Companions or * to pick.",page+1,npage), &ch)) break;
		if(ch != ' ') break;
		page = (page+1)%npage;
		Term_load();
	}
	...
	if(ch >= 'a' && ch <= 'z')
	{
		chosen = page * nper - (int)'a' + (int)ch;
		for(i = 0; i < max_m_idx; i++)
		{
			if(m_list[i].status >= MSTATUS_FRIEND)
				if(counter++ == chosen) return i+5;
		}
	}
	return -1;
}

bool dismiss_pet(int m_idx)
{
	monster_type *m_ptr;
	monster_race *r_ptr;
	...
	if ((!(r_ptr->flags7 & RF7_NO_DEATH)) && (m_ptr->status >= MSTATUS_FRIEND))	/* Get rid of it! */
	{
		if(m_ptr->status == MSTATUS_COMPANION)
		{
			...
			if (!get_check(check_friend)) return 0;
		}
