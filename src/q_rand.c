...
static int randquest_loser[] = { 2, -1 };
...
bool is_randloser()
{
	...
	for (i = 0; randquest_loser[i] != -1; i++)
		if (random_quests[dun_level].type == randquest_loser[i]) break;
	if (randquest_loser[i] != -1)
		return (TRUE);
	else
		return (FALSE);
}
...
void loser_death(s32b m_idx, s32b r_idx)
{
	random_quests[dun_level].type = 0;
	random_quests[dun_level].done = TRUE;

	cmsg_print(TERM_YELLOW, "The loser steps out of the shadows and picks up his intelligence:");
	cmsg_print(TERM_YELLOW, "'Ah! My intelligence! My trusty wits! Thanks.");
	...
	cmsg_print(TERM_YELLOW, "I will now help you in your adventures.'");
...
bool quest_random_death_hook(char *fmt)
{
	...
	if (quest[QUEST_RANDOM].data[0] == random_quests[dun_level].type)
	{
		...
		else if (is_randloser())
			loser_death(m_idx, r_idx);
		else
...
bool quest_random_feeling_hook(char *fmt)
{
	...
	{
		cmsg_format(TERM_YELLOW, "A strange man with drool dripping from his chin, wrapped in a dark cloak steps out of the shadows:");
		cmsg_format(TERM_YELLOW, "'Durr.... A horrible %s stole my intelligence! Um...'", r_info[random_quests[dun_level].r_idx].name + r_name);
	}
	else
...
bool quest_random_gen_loser_hook(char *fmt)
{
	int i;
...
bool quest_random_init_hook(int q_idx)
{
	...
	add_hook(HOOK_LEVEL_END_GEN, quest_random_gen_loser_hook, "rand_gen_loser");
