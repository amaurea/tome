...
bool carried_make_attack_normal(int r_idx)
{
	...
	for (ap_cnt = 0; ap_cnt < 4; ap_cnt++)
	{
		...
		if (!effect || check_hit(power, rlev))
		{
			...
			case RBE_UNLIGHT:
				{
					do_player_unlight(damage, -1, FALSE);
					break;
				}
			case RBE_HALLU:
				{
...
bool make_attack_normal(int m_idx, byte divis)
{
	...
	for (ap_cnt = 0; ap_cnt < 4; ap_cnt++)
	{
		...
		/* React to attempted attack on player.
		 * This may change the monster's status; it could even
		 * be deleted after this (i.e. m_idx is invalid now).
		 * Also, what happens if this is hooked several times?
		 * Then each hook need to check if the monster has died,
		 * or ones that kill the monster must return TRUE to
		 * indicate that the hook chain should be stopped.
		 * Also, what about the order of this? It matters if
		 * armor of fear comes first or if counterattacks come
		 * first, for example. If order really matters, one should
		 * just make one single hook, and implement the order inside
		 * it.  */
		interrupt = process_hooks(HOOK_PLAYER_ATTACKED, "(d,d)", m_idx, ap_cnt);
		if(!m_live(m_idx)) alive = FALSE;

		if(!alive) break;

		/* Too afraid or stunned to attack? */
		if (m_ptr->monfear || m_ptr->stunned) break;

		/* Attack was prevented, but it still gets more chances */
		...
		if (!effect || check_hit(power, rlev))
		{
			...
			case RBE_UN_POWER:
				{
					...
					for (k = 0; k < 10; k++)
					{
						...
						if((o_ptr->tval == TV_STAFF || o_ptr->tval == TV_WAND) &&
							!artifact_p(o_ptr) && !(f5 & TR5_INSULATE) &&
							(!p_ptr->resist_disen || magik(33)))
						{
							/* Message */
			...
			case RBE_UNLIGHT:
				{
					do_player_unlight(damage, m_idx, FALSE);
					break;
				}
			}
			...
			if (do_vampire)
			{
				...
				if (magik(2) || (magik(m_ptr->level - (p_ptr->lev * 2))))
