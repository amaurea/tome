	...
	*ecp = 0;
	...
	if (info & (CAVE_MARK | CAVE_SEEN))
	{
		...
		if (!avoid_other && !avoid_shimmer && attr_mutable)
		{
			...
			s16b eff = random_effect(c_ptr);
			if(eff) a = spell_color(effects[eff].type);
			/* Multi-hued attr */
			...
			if (m_ptr->ml)
			{
				...
				*ecp = 0;
				...
				if (use_graphics)
				{
					...
					if (graf_new)
					{
						monster_ego *re_ptr = &re_info[m_ptr->ego[0]];
		...
		case GRAPHICS_IBM:
			{
				if (player_char_health)
				{
					...
					int percent = spectre ? p_ptr->csp * 10 / p_ptr->msp :p_ptr->chp * 10 / p_ptr->mhp;
		...
		case GRAPHICS_NEW:
			{
				...
				{
					int spectre = PRACE_FLAG(PR1_SEMI_WRAITH);
					int percent = spectre ? p_ptr->csp * 14 / p_ptr->msp :p_ptr->chp * 10 / p_ptr->mhp;
 ...
 */
void monster_race_track(int r_idx, u16b * ego)
{
	/* Save this monster ID */
	int i;
	monster_race_idx = r_idx;
	for(i = 0; i < MEGO_MAX_STACK; i++)
		monster_ego_idx[i] = ego[i];
...
int new_effect(int type, int dam, int time, int cy, int cx, int rad, int who, int speed, s32b flags)
{
	int i;
	...
	/* Since project does damage on the turn of casting,
	 * the generated effect counts as just having spent a
	 * move */
	effects[i].energy = -100;
	return i;
}

/* This helper function compares the power of
 * two effects. This is used to termine which
 * of two effects 'win' */
int compare_effect(int a, int b)
{
	int apow = effects[a].dam*extract_energy[effects[a].speed],
		bpow = effects[b].dam*extract_energy[effects[b].speed];
	if(apow > bpow) return 1;
	else if(apow == bpow) return 0;
	else return -1;
}

int strongest_effect(cave_type * c_ptr)
{
	int i, b = 0;
	for(i = 1; i < MAX_EFFECTS_TILE; i++)
		if(!c_ptr->effects[b] || c_ptr->effects[i] &&
			compare_effect(c_ptr->effects[i], c_ptr->effects[b]) > 0)
			b = i;
	return c_ptr->effects[b];
}

int random_effect(cave_type * c_ptr)
{
	int i, n = 0, buf[MAX_EFFECTS_TILE];
	for(i = 0; i < MAX_EFFECTS_TILE; i++)
		if(c_ptr->effects[i]) { buf[n++] = c_ptr->effects[i]; }
	return n ? buf[rand()%n] : 0;
}
