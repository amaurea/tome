...
static bool monst_spell_monst(int m_idx)
{
	...
	monster_race *tr_ptr;
	u32b f4, f5, f6, f9;                        /* racial spell flags */
	bool direct = TRUE;
	bool wake_up = FALSE;
...

	{
		...
		/* Hack: Webs are not in a spell category because
		 * I would have to add a whole new category then */
		if(f9 & RF9_SPIN_WEB) spell[num++] = 32 * 6;
		...
		case 32*6:
			{
				disturb(1, 0);
				if (blind) msg_format("%^s excretes something.", m_name);
				else msg_format("%^s spins webs.", m_name);
				breath(m_idx, GF_WEB, ((m_ptr->hp / 3) > 250 ? 250 : (m_ptr->hp / 3)),
					m_ptr->hp/200 + 1);
				break;
			}
		}
...
bool make_attack_spell(int m_idx)
{
	...
	/* this made the crypt and similar silly, with powerless monsters
	 * caught inside glass walls without being able to do anything,
	 * so I'll do away with it. This introduces the new bug of monsters
	 * trying to cast ball attacks and such through glass walls, but rather
	 * that than nothing working through them. */
	if (normal)
	{
		/* Check range */
		...
		/* Check path */
		if (!los(m_ptr->fy, m_ptr->fx, y, x)) return (FALSE);
	}
	/* Extract the monster level */
	rlev = ((m_ptr->level >= 1) ? m_ptr->level : 1);
...
static bool get_moves(int m_idx, int *mm)
{
	...
	if (!stupid_monsters && (is_friend(m_ptr) < 0) && !(r_ptr->flags7 & RF7_AI_ANNOY))
	{
		int tx = x2, ty = y2;
...
static void process_monster(int m_idx, bool is_frien)
{
	...
	if ((m_ptr->status > MSTATUS_NEUTRAL) && (m_ptr->status < MSTATUS_COMPANION) && !(wizard) && (r_ptr->flags1 & (RF1_UNIQUE)) && !(r_ptr->flags7 & RF7_PET))
		gets_angry = TRUE;
...
void process_monsters(void)
{
	...
	int old_monster_ego_idx[MEGO_MAX_STACK];
	...
	for(i = 0; i < MEGO_MAX_STACK; i++) old_monster_ego_idx[i] = monster_ego_idx[i];
