...
void do_lasting_project()
{
	...
	while(1)
	{
		...
		int n, i, j, o = 1, col;
		char ch;
		for(i = 0, n = 0; i < MAX_EFFECTS; i++)
			if(effects[i].time) n++;

		//c_prt(TERM_WHITE, format("%3d effects currently active", n), o++, 0);
		c_prt(TERM_WHITE, format("   %4s %5s %-16s %6s %8s %6s %3s %3s %4s %4s",
			"id", "owner", "type", "time", "flags", "damage", "spd", "rad", "x", "y"), o++, 0);
		for(i = 0, j = 0; i < MAX_EFFECTS; i++)
		{
			if(effects[i].time == 0) continue;
			if(effects[i].time > 0)
				col = effects[i].who == 0 ? TERM_GREEN : TERM_RED;
			else
				col = effects[i].who == 0 ? TERM_L_GREEN : TERM_L_RED;
			/* id owner type time flags dam speed rad x y */
			c_prt(col, format("%c) %4d %5d %-16s %6d %8x %6d %+3d %3d %4d %4d", 'a'+j,
				i, effects[i].who, get_gf_name(effects[i].type), effects[i].time,
				effects[i].flags, effects[i].dam, effects[i].speed-110, effects[i].rad,
				effects[i].cx, effects[i].cy), o++, 0);
			j++;
		}
		/* Could add proper commands here, for terminating effects, for example.
		 * But a listing is enough for now */
		if (!get_com(format("There are currently %d active effects",n), &ch)) break;
		break;
		Term_load();
	}
	Term_load();
...
void do_monsters()
{
	...
	while(1)
	{
		...
		c_prt(TERM_WHITE, format("   %4s %-24.24s %3s %6s %6s %3s %4s %4s %4s",
			"id", "name", "lvl", "hp", "mhp", "spd", "AC", "x", "y"), o++, 0);
		for(i = 0, j = -page*nper; i < max_m_idx && j < nper; i++)
		{
			...
			c_prt(col, format("%c) %4d %-24.24s %3d %6d %6d %+3d %4d %4d %4d", 'a'+j-1,
				i, desc, m_list[i].level, m_list[i].hp, m_list[i].maxhp, m_list[i].speed-110,
				m_list[i].ac, m_list[i].fx, m_list[i].fy), o++, 0);
		}
		/* Could add proper commands here, for terminating effects, for example.
		 * But a listing is enough for now */
		if (!get_com(format("Current monsters (page %d of %d)",page+1,npage), &ch)) break;
		if(ch != ' ') break;
		page = (page+1)%npage;
		Term_load();
	}
