...
bool mego_ok(int r_idx, u16b ego)
{
...
/* Choose an ego type */
void pick_ego_monster(int r_idx, u16b * egos)
{
	/* Assume no ego */
	int nego = 0;
	int lvl, ego = 0;
	int tries = max_re_idx + 10, tries2;
	memset(egos, 0, MEGO_MAX_STACK);
	monster_ego *re_ptr;

	if (dungeon_flags2 & DF2_ELVEN && nego < MEGO_MAX_STACK && (ego = test_mego_name("Elven")) && mego_ok(r_idx, ego))
		egos[nego++] = ego;
	if (dungeon_flags2 & DF2_DWARVEN && nego < MEGO_MAX_STACK && (ego = test_mego_name("Dwarven")) && mego_ok(r_idx, ego))
		egos[nego++] = ego;

	/* No townspeople ego */
	if (!r_info[r_idx].level) return;

	/* First are we allowed to find an ego */
	while(nego < MEGO_MAX_STACK)
	{
		if (!magik(MEGO_CHANCE)) return;
 ...
 */
monster_race* race_info_idx(int r_idx, u16b * ego)
{
	...
	/* No work needed */
	if (!ego || !ego[0]) return r_ptr;

	/* Copy the base monster */
	COPY(nr_ptr, r_ptr, monster_race);

	for(ei = 0; ei < MEGO_MAX_STACK && ego[ei]; ei++)
	{
		monster_ego *re_ptr = &re_info[ego[ei]];
		/* Adjust the values */
		for (i = 0; i < 4; i++)
		{
			s32b j, k;
...
void delete_monster_idx(int i)
{
	...
	/* Remove all effects bound to this monster */
...
static void compact_monsters_aux(int i1, int i2)
{
	...
	/* Hack -- Update the effects */
	for (j = 0; j < MAX_EFFECTS; j++)
		if(effects[j].who == i1) effects[j].who = i2;
...
void compact_monsters(int size)
{
	...
	/* Really inefficient. Much better to
	 * have an insertion index and deletion index,
	 * and make one pass through in parallel with these */
	for (i = m_max - 1; i >= 1; i--)
	{
...
bool m_live(s16b m_idx)
{
	return m_idx >= max_m_idx ? FALSE : m_list[m_idx].r_idx ? TRUE : FALSE;
}
...
s16b get_mon_num(int level)
{
	...
	for (i = 0; i < alloc_race_size; i++)
	{
		...
		/* Hack -- "unique" monsters must be "unique". But this can be
		 * turned off. */
		if (!allow_clones && r_ptr->flags1 & RF1_UNIQUE &&
		...
		/* Unallow some uniques to be generated outside of their areas */
		if (r_ptr->flags9 & RF9_SPECIAL_GENE && !m_allow_special[r_idx])
			continue;

		/* Disallow Spirits outside The Void. */
		...
		if (r_ptr->on_saved && !allow_clones)
			continue;

		/* No uniques allowed? */
		if(d_ptr->flags2 & DF2_NO_UNIQUES && r_ptr->flags1 & RF1_UNIQUE)
			continue;

		/* No nonunique allowed? */
		if(d_ptr->flags2 & DF2_NO_NONUNIQUES && !(r_ptr->flags1 & RF1_UNIQUE))
			continue;

		/* In some dungeons, the rules for extinction are reversed: Only
		 * killed monsters may be encountered. Sadly, the game uses the
		 * level-local r_ptr->max to avoid duplicate uniques, not the
		 * global number killed variable. This makes the implementation
		 * of this a bit hacky. */
		if(d_ptr->flags2 & DF2_INVERT_LIFE)
		{
			/* We must have killed this monster for it to appear here */
			if(r_ptr->r_pkills == 0 || r_ptr->flags1 & RF1_UNIQUE &&
					!allow_clones && r_ptr->cur_num > 0)
				continue;
			/* Have we run out of dead monsters of this type */
			if(!allow_clones && r_ptr->cur_num >= r_ptr->r_pkills)
				continue;
		}
		else
		{
			if (r_ptr->flags1 & RF1_UNIQUE && r_ptr->max_num == -1 && !m_allow_special[r_idx])
				continue;
			if (r_ptr->flags1 & RF1_UNIQUE && r_ptr->cur_num >= r_ptr->max_num && r_ptr->max_num != -1 && !bypass_r_ptr_max_num && !allow_clones)
				continue;
		}
...
void monster_desc(char *desc, monster_type *m_ptr, int mode)
{
	...
	char silly_name[80], name[0x200], pname[0x200];
	bool seen, pron;
	int i;
	int insanity = (p_ptr->msane - p_ptr->csane) * 100 / p_ptr->msane;

	sprintf(name, "%s", b_name);
	for(i = 0; i < MEGO_MAX_STACK && m_ptr->ego[i]; i++)
	{
		if (re_info[m_ptr->ego[i]].before) sprintf(pname, "%s %s", re_name + re_info[m_ptr->ego[i]].name, name);
		else sprintf(pname, "%s %s", name, re_name + re_info[m_ptr->ego[i]].name);
		strcpy(name, pname);
	}
...
void monster_race_desc(char *desc, int r_idx, u16b * egos)
{
	monster_race *r_ptr = &r_info[r_idx];
	cptr b_name = (r_name + r_ptr->name);
	char name[0x200], pname[0x200];
	int i;

	sprintf(name, "%s", b_name);
	for(i = 0; egos && i < MEGO_MAX_STACK && egos[i]; i++)
	{
		if (re_info[egos[i]].before) sprintf(pname, "%s %s", re_name + re_info[egos[i]].name, name);
		else sprintf(pname, "%s %s", name, re_name + re_info[egos[i]].name);
		strcpy(name, pname);
	}
...
s16b place_monster_one(int y, int x, int r_idx, u16b * ego, bool slp, int status)
{
	int i, base;
	char dummy[5];
	...
	if (!ego_uniques && r_ptr->flags1 & RF1_UNIQUE && ego && ego[0])
	{
		if (place_monster_one_race) KILL(place_monster_one_race, monster_race);
		return 0;
	}
	...
	/* These checks are not based on location, and should be duplicated
	 * in get_mon_num for efficiency */
	...
	/* Disallow Spirits outside The Void, now this *IS* an ugly hack, I hate to do it.. */
	if ((r_ptr->flags7 & RF7_SPIRIT) && (dungeon_type != DUNGEON_VOID))
	{
	 ...
	 * global number killed variable. This makes the implementation
	 * of this a bit hacky. */
	/* I don't understand what max_num == -1 means */
	if(d_ptr->flags2 & DF2_INVERT_LIFE)
	{
		/* We must have killed this monster for it to appear here */
		/* But only as many as we have killed can appear, unless allow
		 * clones is set */
		if(r_ptr->r_pkills == 0)
		{
			if (wizard) cmsg_format(TERM_L_RED, "WARNING: Refused monster %d: Not killed yet", r_idx);
			if (place_monster_one_race) KILL(place_monster_one_race, monster_race);
			return 0;
		}
		if(!allow_clones && r_ptr->cur_num >= r_ptr->r_pkills)
		{
			//if (wizard) cmsg_format(TERM_L_RED, "WARNING: Refused monster %d: Not enough killed", r_idx);
			if (place_monster_one_race) KILL(place_monster_one_race, monster_race);
			return 0;
		}
		if(r_ptr->flags1 & RF1_UNIQUE && !allow_clones && r_ptr->cur_num > 0)
		{
	...
	/* Save the race */
	m_ptr->r_idx = r_idx;
	for(i = 0; i < MEGO_MAX_STACK; i++) m_ptr->ego[i] = ego ? ego[i] : 0;
	...
	/* Hack: Unlight storm */
	if (r_ptr->flags7 & RF7_UNLIGHT)
	{
		new_effect(GF_UNLIGHT, m_ptr->level*2, -1, m_ptr->fy, m_ptr->fx,
			1+m_ptr->level/25, c_ptr->m_idx, r_ptr->speed, EFF_STORM);
	}
...
static bool place_monster_group(int y, int x, int r_idx, bool slp, int status)
{
	...
	for (n = 0; (n < hack_n) && (hack_n < total); n++)
	{
		...
		for (i = 0; (i < 8) && (hack_n < total); i++)
		{
			...
			pick_ego_monster(r_idx, egos);
			if (place_monster_one(my, mx, r_idx, egos, slp, status))
			{
...
bool place_monster_aux(int y, int x, int r_idx, bool slp, bool grp, int status)
{
	...
	if (!place_monster_one(y, x, r_idx, egos, slp, status)) return (FALSE);
	...
	if (r_ptr->flags1 & (RF1_ESCORT))
	{
		...
		for (i = 0; i < 50; i++)
		{
			...
			pick_ego_monster(z, egos);
			place_monster_one(ny, nx, z, egos, slp, status);
