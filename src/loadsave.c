...
static bool do_extra(int flag)
{
	...
	for (i = 0; i < tmp16s; ++i)
	{
		if (i < old_max_s_idx)
		{
			do_s32b(&s_info[i].raw, flag);
			if(vernum < 105 && flag == LS_LOAD)
				must_fix_skills = TRUE;
...
static void do_monster(monster_type *m_ptr, int flag)
{
	...
	if(flag == LS_LOAD)
		for(i = 0; i < MEGO_MAX_STACK; i++)
			m_ptr->ego[i] = 0;
	if(flag == LS_LOAD && vernum < 106) do_u16b(m_ptr->ego, flag);
	else for(i = 0; i < MEGO_MAX_STACK; i++) do_u16b(m_ptr->ego+i, flag);
...
static bool do_dungeon(int flag)
{
	...
	/* Compact stuff. monsters must be done before effects,
	 * since effects may refer to monster indices */
	...
	for (i = 0; i < tmp16b; ++i)
	{
		...
		do_u16b(&effects[i].who, flag);
		do_u16b(&effects[i].speed, flag);
	}
	...
	if(flag == LS_SAVE || vernum > 103) do_grid_new(flag);
	else do_grid(flag);
	...
	for (part = 0; part < 9; part++)
	{
		for (y = 0; y < ymax; y++)
		{
			for (x = 0; x < xmax; x++)
			{
				...
				switch (part)
				{
				case 0: do_u16b(&c_ptr->info, flag); break; 
				case 1: do_byte(&c_ptr->feat, flag); break; 
				case 2: do_byte(&c_ptr->mimic, flag); break; 
				case 3: do_u16b(&c_ptr->special, flag); break; 
				case 4: do_u16b(&c_ptr->special2, flag); break; 
				case 5: do_u16b(&c_ptr->t_idx, flag); break; 
				case 6: do_u16b(&c_ptr->inscription, flag); break; 
				case 7: do_byte(&c_ptr->mana, flag); break; 
				case 8: do_u16b(&c_ptr->effect, 12, 0, flag); break; }
			}
		}
	}
		...
		if (flag == LS_SAVE)
		{
			...
			for (y = 0; y < cur_hgt; y++)
			{
				for (x = 0; x < cur_wid; x++)
				{
					...
					switch (part)
					{
					case 0: tmp16s = c_ptr->info; break; 
					case 1: tmp8u = c_ptr->feat; break; 
					case 2: tmp8u = c_ptr->mimic; break; 
					case 3: tmp16s = c_ptr->special; break; 
					case 4: tmp16s = c_ptr->special2; break; 
					case 5: tmp16s = c_ptr->t_idx; break; 
					case 6: tmp16s = c_ptr->inscription; break; 
					case 7: tmp8u = c_ptr->mana; break; 
					case 8: tmp16s = c_ptr->effects[0]; break; }
					/* Flush a full run */
					if ((((part != 1) && (part != 2) && (part != 7)) &&
						(tmp16s != prev_s16b)) || (((part == 1) || (part == 2)
						|| (part == 7)) && (tmp8u != prev_char)) || (count == MAX_UCHAR))
					{
		...
		if (flag == LS_LOAD)
		{
			...
			for (y = 0; y < ymax; )
			{
				...
				for (i = count; i > 0; i--)	/* RLE */
				{
					...
					switch (part)
					{
					case 0: c_ptr->info = tmp16s; break; 
					case 1: c_ptr->feat = tmp8u; break; 
					case 2: c_ptr->mimic = tmp8u; break; 
					case 3: c_ptr->special = tmp16s; break; 
					case 4: c_ptr->special2 = tmp16s; break; 
					case 5: c_ptr->t_idx = tmp16s; break; 
					case 6: c_ptr->inscription = tmp16s; break; 
					case 7: c_ptr->mana = tmp8u; break; 
					case 8: c_ptr->effects[0] = tmp16s;
						for(j = 1; j < MAX_EFFECTS_TILE; j++)
							c_ptr->effects[j] = 0;
						break;
					}
					if (++x >= xmax)
...
static void do_grid_new(int flag)
{
	int x, y, i, neff;
	cave_type *c_ptr;
	int ymax = cur_hgt, xmax = cur_wid;
	my_sentinel("Before grid", 17, flag);
	if(flag == LS_SAVE) neff = MAX_EFFECTS_TILE;
	do_u16b(&neff, flag);
	for(y = 0; y < ymax; y++)
		for(x = 0; x < xmax; x++)
		{
			...
			for(i = 0; i < neff && i < MAX_EFFECTS_TILE; i++)
				do_u16b(&c_ptr->effects[i], flag);
			for(; i < MAX_EFFECTS_TILE; i++) c_ptr->effects[i] = 0;
		}
	my_sentinel("In grid", 36, flag);
}
