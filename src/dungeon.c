 ...
 * This one does not use modify_sp, as it would be called too often.
	...
	case CLASS_CANALIZE_MANA:
		{
			...
			{
				modify_sp(cave[p_ptr->py][p_ptr->px].mana / 10);
			}
			else
			{
				modify_sp(cave[p_ptr->py][p_ptr->px].mana / 20);
			}
...
static void process_world(void)
{
	...
	if (!cave_floor_bold(p_ptr->py, p_ptr->px))
	{
		...
		else if (PRACE_FLAG(PR1_SEMI_WRAITH) && (!p_ptr->wraith_form) && (f_info[cave[p_ptr->py][p_ptr->px].feat].flags1 & FF1_CAN_PASS))
		{
			...
			cave_no_regen = TRUE;
			if (2*amt > p_ptr->csp) amt = p_ptr->csp/2;
			take_hit(amt, " walls ...");
		}
	}
	...
	/* Regenerate the mana */
	/* no regen now means no regen, even for mana */
	if (p_ptr->csp < p_ptr->msp && !cave_no_regen)
	{
		if (upkeep_factor)
		{
	...
	/* Roots help */
	if (p_ptr->tim_roots) regen_amount += 200 + wisdom_scale(800);
	...
	PRAY_GOD(GOD_YAVANNA)
	{
		...
		{
			regen_amount += 500 + wisdom_scale(1500);
		}
	}
	...
	/* Timed mimicry */
	if (FALSE && get_skill(SKILL_MIMICRY))
	{
...
duneffecthack:
	...
	if (!p_ptr->wild_mode)
	{
		...
		for (i = 0; i < MAX_EFFECTS; i++)
		{
			...
			effects[i].energy += extract_energy[effects[i].speed];
			if (effects[i].energy < 0) continue;

			/* Reduce duration. Negative time equals unlimited.
			 * Be careful with this, as this could cause effects
			 * to pile up. */
			if(effects[i].time > 0) effects[i].time--;
			...
			else if (effects[i].flags & EFF_STORM)
			{
				...
				else if(!m_list[e_ptr->who].r_idx) e_ptr->time = 0;
				else
				{
					e_ptr->cy = m_list[e_ptr->who].fy;
					e_ptr->cx = m_list[e_ptr->who].fx;
				}
				for (y = e_ptr->cy - e_ptr->rad; y <= e_ptr->cy + e_ptr->rad; y++)
				{
					for (x = e_ptr->cx - e_ptr->rad; x <= e_ptr->cx + e_ptr->rad; x++)
		...
		/* Make effect wait for its next turn */
		for (i = 0; i < MAX_EFFECTS; i++)
			if (effects[i].time != 0 && effects[i].energy >= 0)
				effects[i].energy -= 100;
	}
	if(skipturn) return;
	...
	{
		amount = (magik(30) ? 2 : 1)*p_ptr->drain_mana;
		if(p_ptr->csp < amount) disturb(0,0);
		modify_sp(-amount);
	}
	...
	if (p_ptr->maintain_sum)
	{
		...
		amount = p_ptr->maintain_sum / 10000;
		if(amount > p_ptr->csp)
		{
			disturb(0,0);
			p_ptr->maintain_sum = 0;
		}
		else p_ptr->maintain_sum -= amount * 10000;
		modify_sp(-amount);
	}
	...
	if (p_ptr->drain_life)
	{
		...
		modify_hp(drain < p_ptr->chp ? drain : p_ptr->chp, "drained of life by summons");
		if (p_ptr->chp == 0) disturb(0, 0);
	}
...
/* Tile effect manipulation */
int add_effect_tile(cave_type * c_ptr, int effect)
{
	int i;
	// Overwrite if possible
	for(i = 0; i < MAX_EFFECTS_TILE; i++)
		if(c_ptr->effects[i] && effects[c_ptr->effects[i]].type == effects[effect].type)
		{
			c_ptr->effects[i] = effect;
			return TRUE;
		}
	// Otherwise, use first clear spot
	for(i = 0; i < MAX_EFFECTS_TILE; i++)
		if(!c_ptr->effects[i])
		{
			c_ptr->effects[i] = effect;
...
void clear_effect_tile(cave_type * c_ptr, int index)
{
	c_ptr->effects[index] = 0;
}
void clear_all_effects_tile(cave_type * c_ptr)
{
	int i;
	for(i = 0; i < MAX_EFFECTS_TILE; i++) c_ptr->effects[i] = 0;
}
